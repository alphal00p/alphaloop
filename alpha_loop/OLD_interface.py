...
class alphaLoopInterface(madgraph_interface.MadGraphCmd, cmd.CmdShell):
    ...
    def do_display_alphaLoop_option(self, line):
        ...
        logger.info('%sGeneral alphaLoop options%s' %
    ...
    def do_display_FORM_option(self, line):
        ...
        logger.info('%sGeneral FORM processing options%s' %
                    (utils.bcolors.GREEN, utils.bcolors.ENDC))
        logger.info('%s-----------------------%s' %
                    (utils.bcolors.GREEN, utils.bcolors.ENDC))
    ...
    def do_set_FORM_option(self, line):
        ...
        if key in self.FORM_options:
            ...
            raise alphaLoopInvalidCmd(
                "Specified FORM option '%s' not reckognized." % key)
    ...
    def do_set_alphaLoop_option(self, line):
        ...
        if key == 'virtual_memory':
            ...
            vlimit_conv = {'B': 1, 'K': 1024, 'M': 1024 **
                           2, 'G': 1024**3, 'T': 1024**3}
            if vlimit_format.match(value) is None and value != '-1':
                raise alphaLoopInvalidCmd(
                    "Virtual memory limit had to be an integer in bytes (possible explicit units B, K, M, G, T)")
            ...
            try:
                ...
                raise alphaLoopInvalidCmd(
                    "Specified perturbative orders should be a dictionary of orderrs, not '%s'." % value)
            for order, v in orders_dict.items():
                if order not in self._curr_model['order_hierarchy'].keys():
                    raise alphaLoopInvalidCmd(
                        "Specified order is not part of the model '%s'." % order)
                if v % 2 != 0:
                    raise alphaLoopInvalidCmd(
                        "Specified perturbative orders shoudl be multiple of 2, not '%d'." % v)
            ...
            if value.upper() == 'NONE':
                ...
                try:
                    ...
                    raise alphaLoopInvalidCmd(
                        "alphaLoop option 'n_jets' should either be None or an integer, not '%s'." % value)
            ...
            if value.upper() == 'NONE':
                ...
                try:
                    ...
                    raise alphaLoopInvalidCmd(
                        "alphaLoop option 'final_state_pdgs' should either be None or a list/tuple of integer, not '%s'." % value)
            self.alphaLoop_options[key] = tuple(fs_pdgs)
        elif key == 'include_denominators':
            if value.upper() not in ['TRUE', 'FALSE']:
                raise alphaLoopInvalidCmd(
                    "Specified value for 'include_denominators' should be 'True' or 'False', not '%s'." % value)
            ...
            if value.upper() not in ['TRUE', 'FALSE']:
                raise alphaLoopInvalidCmd(
                    "Specified value for 'use_physical_gluon_helicity_sum' should be 'True' or 'False', not '%s'." % value)
            bool_val = (value.upper() == 'TRUE')
            self.alphaLoop_options['use_physical_gluon_helicity_sum'] = bool_val
        elif key == 'loop_induced':
            if value.upper() not in ['TRUE', 'FALSE']:
                raise alphaLoopInvalidCmd(
                    "alphaLoop option 'loop_induced' should be 'True' or 'False', not %s" % value)
            self.alphaLoop_options['loop_induced'] = value.upper() == 'TRUE'
        elif key == 'qgraf_cut_filter':
            if value.upper() not in ['TRUE', 'FALSE']:
                raise alphaLoopInvalidCmd(
                    "Specified value for '%s' should be 'True' or 'False', not '%s'." % (key, value))
            bool_val = (value.upper() == 'TRUE')
            ...
            if not value.isdigit():
                raise alphaLoopInvalidCmd("alphaLoop option 'FORM_compile_cores' takes values form 1 to %d. Not %s" % (
                    multiprocessing.cpu_count(), value))
            cores = int(value)
            if cores > multiprocessing.cpu_count():
                cores = multiprocessing.cpu_count()
                logger.warning(
                    "Asking from more CORES then available. Overwriting the value to %s" % cores)
            self.FORM_options['cores'] = cores
        elif key == 'FORM_compile_arg':
            if not value in ['integrand', 'numerator', 'all']:
                raise alphaLoopInvalidCmd(
                    "alphaLoop option 'FORM_compile_output' should be one of 'numerator', 'integrand', 'all'")
            ...
            if not value in [str(opt_n) for opt_n in range(4)]:
                raise alphaLoopInvalidCmd(
                    "alphaLoop option 'FORM_compile_optimization' should be between 0 and 3, not %s" % value)
            self.FORM_options['compilation-options'] += ['-e',
                                                         "OPTIMIZATION_LVL=%s" % value]
            self.FORM_options['extra-options']['OPTIMLVL'] = [1,
                                                              2, 4, 4][int(value)]
        elif key == 'FORM_compile_optimization_f128':
            if not value in [str(opt_n) for opt_n in range(4)]:
                raise alphaLoopInvalidCmd(
                    "alphaLoop option 'FORM_compile_optimization_f128' should be between 0 and 3, not %s" % value)
            self.FORM_options['compilation-options'] += ['-e',
                                                         "OPTIMIZATION_LVL_f128=%s" % value]
        elif key == 'FORM_integrand_type':
            if not value in ('PF', 'LTD', 'both', 'None'):
                raise alphaLoopInvalidCmd(
                    "alphaLoop option 'FORM_integrand_type' should be one of 'PF', 'LTD', 'None', not %s" % value)
            self.alphaLoop_options['FORM_integrand_type'] = value if value in (
                'PF', 'both', 'LTD') else None
        elif key == 'FORM_generate_arb_prec_output':
            if value.upper() not in ['TRUE', 'FALSE']:
                raise alphaLoopInvalidCmd(
                    "alphaLoop option 'FORM_generate_arb_prec_output' should be a bool', not %s" % value)
            self.FORM_options['generate_arb_prec_output'] = True if value.upper(
            ) == 'TRUE' else False
            if value.upper() == 'TRUE':
                self.FORM_options['compilation-options'] += ['-e',
                                                             "ARB_PREC=1"]
            ...
            if not value in ('nosum', 'onlysum', 'both'):
                raise alphaLoopInvalidCmd(
                    "alphaLoop option 'FORM_sum_diagram_sets' should be one of 'nosum', 'onlysum', 'both', not %s" % value)
            self.alphaLoop_options['FORM_sum_diagram_sets'] = value if value in (
                'nosum', 'onlysum', 'both') else None
            ...
            if value.upper() not in ['TRUE', 'FALSE']:
                raise alphaLoopInvalidCmd(
                    "Specified value for 'include_self_energies_from_squared_amplitudes' should be 'True' or 'False', not '%s'." % value)
            bool_val = (value.upper() == 'TRUE')
...
set to False, except for debugging, which seems to be what you are doing now, so we'll se it to True now.%s""" % (
            ...
            if not parsed_value is None:
                if (not (isinstance(parsed_value, list) or isinstance(parsed_value, tuple))) or \
                    ...
                    raise alphaLoopInvalidCmd(
                        "Invalid value for option supergraphs_selection: '%s'" % value)
            self.alphaLoop_options['supergraphs_selection'] = parsed_value

        elif key == 'n_rust_inputs_to_generate':
            try:
                self.alphaLoop_options['n_rust_inputs_to_generate'] = int(
                    value)
            except ValueError:
                raise alphaLoopInvalidCmd(
                    "Specified value for 'n_rust_inputs_to_generate' should be an integer, not '%s'." % value)
        elif key == 'checkpoint_lvl':
            if not value in [str(opt_n) for opt_n in range(4)]:
                raise alphaLoopInvalidCmd(
                    "alphaLoop option 'checkpoint_lvl' should be between 0 and 3, not %s" % value)
            ...
            try:
                ...
                raise alphaLoopInvalidCmd(
                    "'SG_name_list' must be of the form [\"name_SG1\",...,\"name_SGn\"] not '%s'." % value)
        else:
            raise alphaLoopInvalidCmd(
                "Unrecognized alphaLoop option: %s" % key)
    ...
    def do_prepare_model_for_self_energies(self, line):
        ...
        if len(args) != 2:
            raise alphaLoopInvalidCmd(
                "Supply exactly two arguments to the command 'do_prepare_model_for_self_energies'.")
        else:
            if args[0] == 'revert':
                ...
                self.prepare_model_for_self_energies(
                    anchor_vertices, revert=False, perturbative_order=n_SE)
    ...
    def prepare_model_for_self_energies(self, anchor_vertices, revert=False, perturbative_order=3):
        ...
        if revert:
            ...
            logger.info('Self-energy patch for model %s successfully reverted.' %
                        self.model_backup_copy.get('name'))
        ...
        pure_QCD_corrections_only = list(
            self.alphaLoop_options['perturbative_orders'].keys()) == ['QCD',]
        ...
        for n_self_energy in range(1, perturbative_order+1):
            if (not model['order_hierarchy']) or any(order in [
                ...
                raise MadGraph5Error(
                    "Cannot prepare the currently active model for self-energies.")
            model['order_hierarchy']['SE_%d_ANCHOR' % n_self_energy] = 0
            for order in self.alphaLoop_options['perturbative_orders']:
                model['order_hierarchy']['SE_%d_%s' %
                                         (n_self_energy, order)] = model['order_hierarchy'][order]
            for n_bridge in range(1, perturbative_order+1):
                if (not model['order_hierarchy']) or any(order in [
                    ...
                    raise MadGraph5Error(
                        "Cannot prepare the currently active model for self-energies.")
                model['order_hierarchy']['SE_%d_BRIDGE_%d' %
                                         (n_self_energy, n_bridge)] = 0
                model['order_hierarchy']['SE_%d_CONTACT_%d' %
                                         (n_self_energy, n_bridge)] = 0
        ...
        for n_self_energy in range(1, perturbative_order+1):
            ...
            for particle in model['particles']:
                ...
                new_particle['pdg_code'] = particle['pdg_code'] + \
                    n_self_energy*pdg_offset
                new_particle['name'] = ('SE_%d_' %
                                        n_self_energy+particle['name']).lower()
                new_particle['antiname'] = (
                    'SE_%d_' % n_self_energy+particle['antiname']).lower()
                new_particles[n_self_energy][particle.get_pdg_code()
                                             ] = new_particle
                if not new_particle['self_antipart']:
                    ...
                    new_particles[n_self_energy][particle.get_anti_pdg_code(
                    )] = new_anti_particle
        ...
        all_anchor_vertices = []
        sorted_selected_anchor_vertices = [
            sorted(pdgs) for pdgs in anchor_vertices]
        for inter in model['interactions'].get_type('base'):
            particle_pdgs = sorted([p.get_pdg_code()
        ...
        for n_self_energy in range(1, perturbative_order+1):
            ...
            for inter in model['interactions'].get_type('base'):
                ...
                for order in new_interaction['orders']:
                    new_orders['SE_%d_%s' % (
                        n_self_energy, order)] = new_interaction['orders'][order]
                ...
                for i_leg in range(len(new_interaction['particles'])):
                    ...
                    a_new_interaction['particles'] = base_objects.ParticleList([
                        new_particles[n_self_energy][p.get_pdg_code(
                        )] if i_p != i_leg else p
                        for i_p, p in enumerate(new_interaction['particles'])
                    ]+[new_particles[n_self_energy]['anchor_0'],]
                    )
                    a_new_interaction['lorentz'] = ['SE_%s' %
                                                    l for l in new_interaction['lorentz']]
                    new_lorentz_names = []
                    for l in new_interaction['lorentz']:
                        ...
                        if new_lorentz_name not in new_lorentz_structures:
                            new_lorentz_struct = copy.deepcopy(
                                model.get_lorentz(l))
                ...
                new_SE_sectors_interactions[n_self_energy].append(
                    new_interaction)
            ...
            for n_bridge in range(1, perturbative_order+1):
                new_interactions.append(base_objects.Interaction({
                    ...
                    'particles': base_objects.ParticleList([
                        new_particles[n_self_energy]['anchor_%d' %
                                                     (n_bridge-1)],
                ...
                for inter in new_SE_sectors_interactions[n_self_energy]:
                    ...
                    new_orders['SE_%d_CONTACT_%d' %
                               (n_self_energy, n_bridge)] = 1
                    new_interaction.set('orders', new_orders)
                    new_interaction['particles'] = base_objects.ParticleList([
        ...
        logger.info(
            'Model %s successfully patched so as to accomodate self-energy generations.' % model.get('name'))
        return
    ...
    def complete_launch(self, text, line, begidx, endidx, formatting=True):
        ...
        if args[-1].endswith(os.path.sep):
            return self.path_completion(text,
                                        pjoin(
                                            *[a for a in args if a.endswith(os.path.sep)]),
                                        only_dirs=True)
        # Format
        if len(args) == 1:
            out = {
                'Path from ./': self.path_completion(text, '.', only_dirs=True)}
    ...
    def do_qgraf_generate(self, line):
        ...
        args = self.split_arg(line)
        process_definition = self.extract_process(
            " ".join(args), proc_number=0)
...
#        proc_output_path = pjoin(MG5DIR,args[1])
        ...
        if self._export_format == 'plugin':
            ...
            if self.plugin_output_format_selected == 'alphaLoop':
                ...
                if 'j' not in self._multiparticles:
                    raise alphaLoopInvalidCmd(
                        "alphaLoop requires the 'j' multiparticle label to be defined.")
                # TODO make this 82, -82 general
                self.alphaLoop_options['_jet_PDGs'] = tuple([
                    pdg for pdg in self._multiparticles['j']
                ]+[82, -82])
    ...
    def do_output_LU_scalar(self, line):
        ...
        if len(args) <= 1:
            raise alphaLoopInvalidCmd(
                "Missing mandatory first argument for output LU scalar which is the output directory name.")
        ...
        for arg in opt_args:
            ...
            if key == '--topology':
                ...
                if not isinstance(topology, (tuple, list)):
                    raise alphaLoopInvalidCmd(
                        "Specified topology must be a tuple or a list")
                if any(
                    ((not isinstance(te, (tuple, list))) or (not isinstance(te[0], str)) or (
                        not isinstance(te[1], int)) or (not isinstance(te[2], int)))
                        for te in topology):
                    raise alphaLoopInvalidCmd(
                        "Each element of the topology specified must be a tuple ('edge_name', node_int_id_left, node_int_id_right) ")
            ...
            if key == '--masses':
                ...
                if not isinstance(topology, (tuple, list)):
                    raise alphaLoopInvalidCmd(
                        "Specified masses must be a tuple or a list")
                if any(
                    ((not isinstance(me, (tuple, list))) or (
                        not isinstance(me[0], str)) or (not isinstance(me[1], float)))
                        for me in masses):
                    raise alphaLoopInvalidCmd(
                        "Each element of the masses specified must be a tuple ('edge_name', float_mass) ")
            ...
            if key == '--edge_scalings':
                ...
                if not isinstance(edge_scalings, (tuple, list)):
                    raise alphaLoopInvalidCmd(
                        "Specified edge scalings must be a tuple or a list")
                if any(
                    ((not isinstance(es, (tuple, list))) or (
                        not isinstance(es[0], str)) or (not isinstance(es[1], int)))
                        for es in edge_scalings):
                    raise alphaLoopInvalidCmd(
                        "Each element of the specified edge scalings must be a tuple ('edge_name', int_scaling) ")
            ...
            if key == '--node_scalings':
                ...
                if not isinstance(node_scalings, (tuple, list)):
                    raise alphaLoopInvalidCmd(
                        "Specified node scalings must be a tuple or a list")
                if any(
                    ((not isinstance(ns, (tuple, list))) or (not isinstance(ns[0], (tuple, list)) or not all(
                        isinstance(nse, str) for nse in ns[0])) or (not isinstance(ns[1], int)))
                        for ns in node_scalings):
                    raise alphaLoopInvalidCmd(
                        "Each element of the specified node scalings must be a tuple ( ('edge_nameA', 'edge_nameB', ...) , int_scaling) ")
            ...
            if key == '--externals':
                ...
                if not isinstance(externals, (tuple, list)) or any(not isinstance(el, str) for el in externals):
                    raise alphaLoopInvalidCmd(
                        "Each element of the list of externals must be a string identifying the edge name.")
            ...
            if key == '--lmb':
                ...
                if (lmb is not None) and (not isinstance(lmb, (tuple, list)) or any(not isinstance(el, str) for el in lmb)):
                    raise alphaLoopInvalidCmd(
                        "Each element of the list of lmb must be a string identifying the edge name.")
        ...
        for arg in mandatory_args:
            if processed_args[arg] is None:
                raise alphaLoopInvalidCmd(
                    "Missing mandatory option '%s' for command do_output_LU_scalar." % arg)
        ...
        try:
            ...
            if processed_args['node_scalings'] is not None:
                for e_list, s in processed_args['node_scalings'].items():
                    FORM_processing.forced_node_scalings[tuple(
                        sorted(e_list))] = s
                    FORM_processing.forced_node_scalings[tuple(
                        sorted([('p%s' % e if not e.startswith('q') else e) for e in e_list]))] = s
        ...
        for arg in opt_args:
            ...
            if key == '--externals':
                ...
                if (not isinstance(externals, (tuple, list))) or (not len(externals) == 2) or \
                    ...
                    raise alphaLoopInvalidCmd(
                        "Each element of each of the two lists provided in the 2-tuple 'externals' must be a string identifying the edge name.")
            ...
            if key == '--default_kinematics':
                ...
                if not isinstance(default_kinematics, dict) or any(not isinstance(k, str) for k in default_kinematics.keys()) or \
                    ...
                    raise alphaLoopInvalidCmd(
                        "Keys of the default_kinematics provided must identify the edge name and values must be four-tuples specifying the kinematics.")
            ...
            if key == '--masses':
                ...
                if not isinstance(masses, dict) or any(not isinstance(k, str) for k in masses.keys()) or \
                    ...
                    raise alphaLoopInvalidCmd(
                        "Keys of the masses dictionary provided must identify the edge name and values must be a float specifying the mass.")
    ...
    def do_hello_world(self, line):
        ...
        logger.info('Hello World, and welcome you, %s%s%s!' %
