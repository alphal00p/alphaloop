...
class alphaLoopExporter(export_v4.ProcessExporterFortranSA):
    ...
    matrix_template = pjoin(plugin_src_path, 'Templates',
                            'matrix_standalone.inc')
    ...
    def write_LTD_squared_info(self, matrix_element, proc_number, *args, **opts):
        ...
        is_self_energy = any(abs(l.get(
            'id')) >= LTD_squared.self_energy_global_id_offset for l in process.get('legs'))
        ...
        LTD2_diagram_list = LTD2_diagram_list_class(
            matrix_element, self.alphaLoop_options)
        all_super_graphs = super_graph_list_class(
            LTD2_diagram_list, proc_number, self.alphaLoop_options['supergraphs_selection'])
    ...
    def write_overall_LTD_squared_info(self, *args, **opts):
        ...
        representative_process = self.all_super_graphs[0][1].get('processes')[
            0]
        ...
        for super_graph_list, ME in self.all_super_graphs:
            final_ids = [l.get('id') for l in ME.get('processes')[
                0].get('legs') if l.get('state') is True]
            fs_symm_factor = 1
        ...
        if self.alphaLoop_options['apply_graph_isomorphisms']:
            ...
            logger.info(
                "Detecting isomorphisms between all %d 'unique' super-graphs generated across all processes..." % n_tot_supergraphs)
            i_graph = 0
            with progressbar.ProgressBar(
                ...
                for super_graph_list, ME in self.all_super_graphs:
                    ...
                    for super_graph in list(super_graph_list):
                        ...
                        bar.update(n_unique_super_graphs_sofar='%d' %
                                   len(overall_basis))
                        bar.update(i_graph)
        ...
        is_LO = (not self.alphaLoop_options['perturbative_orders']) or sum(
            self.alphaLoop_options['perturbative_orders'].values()) == 0
        ...
        for all_super_graphs, matrix_element in self.all_super_graphs:
            ...
            base_proc_name = matrix_element.get(
                'processes')[0].shell_string().split('_', 1)[1]
                ...
                for i_super_graph, super_graph in enumerate(all_super_graphs):
                    ...
                    if len(super_graph.cutkosky_cuts_generated) == 0:
                        ...
                        if is_LO:
                            ...
                            reconstructed_symmetry_factor = int(len(
                                super_graph.MG_LO_cuts_corresponding_to_this_supergraph)/len(super_graph.cutkosky_cuts_generated))
...
#                                ) )
                        ...
                        if is_LO:
                            super_graph.symmetry_factor = reconstructed_symmetry_factor / \
                                float(fs_symm_factor)
                        else:
                        ...
                        n_tot_cutkosky_cuts += fs_symm_factor * super_graph.symmetry_factor * \
                            max(len(super_graph.cutkosky_cuts_generated), 0)
                        ...
                        if do_debug:
                            if fs_symm_factor*super_graph.symmetry_factor*len(super_graph.cutkosky_cuts_generated) != len(super_graph.MG_LO_cuts_corresponding_to_this_supergraph):
                                log_func(("%s%s: alphaLoop only found %d (fs_sym) x %d (graph_sym) x %d (n_cuts) LO cutkosky cuts in diagram '%s'%s, whereas the " +
                                    ...
                                    len(super_graph.MG_LO_cuts_corresponding_to_this_supergraph),
                                    ', '.join(
                                        '%dx%d' % interf for interf in super_graph.MG_LO_cuts_corresponding_to_this_supergraph),
                                    utils.bcolors.ENDC
                                ))
        ...
        if self.alphaLoop_options['FORM_processing_output_format']:
            ...
            Path(pjoin(self.dir_path, 'FORM', 'Rust_inputs')).mkdir(
            ...
            if os.path.isfile(pjoin(plugin_path, 'Templates', 'makefile_user_opts.inc')):
                shutil.copy(pjoin(plugin_path, 'Templates', 'makefile_user_opts.inc'), pjoin(
                    self.dir_path, 'FORM', 'makefile_user_opts.inc'))
            else:
                shutil.copy(pjoin(plugin_path, 'Templates', 'makefile_user_opts_default.inc'), pjoin(
                    self.dir_path, 'FORM', 'makefile_user_opts.inc'))
            ...
            for FORM_super_graph in FORM_super_graph_list:
                graph_dumps.append('graph_names.append("%s")' %
                                   FORM_super_graph[0].name)
            ...
            if self.alphaLoop_options['n_jets'] is not None:
                ...
                if len(self.alphaLoop_options['perturbative_orders']) != 0:
                    n_jets -= sum(
                        self.alphaLoop_options['perturbative_orders'].values())//2
        ...
        overall_xsec_yaml_file_path = pjoin(
            rust_inputs_path, '%s.yaml' % base_name)
        overall_xsec_yaml = {
            'name': base_name,
            'topologies': [
                {
        ...
        open(overall_xsec_yaml_file_path, 'w').write(
            yaml.dump(overall_xsec_yaml, Dumper=Dumper, default_flow_style=False))
    ...
    def copy_template(self, model):
        ...
        Path(pjoin(self.dir_path, 'Source', 'MODEL')).mkdir(
            parents=True, exist_ok=True)
        Path(pjoin(self.dir_path, 'Source', 'DHELAS')).mkdir(
            parents=True, exist_ok=True)
        Path(pjoin(self.dir_path, 'SubProcesses')).mkdir(
            parents=True, exist_ok=True)
        Path(pjoin(self.dir_path, 'bin')).mkdir(parents=True, exist_ok=True)
        Path(pjoin(self.dir_path, 'bin', 'internal')).mkdir(
            parents=True, exist_ok=True)
...
#        self.replace_make_opt_f_compiler(compiler)
        ...
        C_binding_replace_dict = self.write_c_bindings(
            writers.FortranWriter(filename))

        # Add the IO_bindings
        filename = pjoin(self.dir_path, 'SubProcesses', 'IO_bindings.f')
        self.write_IO_bindings(writers.FortranWriter(
            filename), C_binding_replace_dict)

        # Compile the C_bindings
        logger.info("Compiling C_bindings for MG numerators...")
        try:
            misc.compile(arg=[], cwd=pjoin(
                self.dir_path, 'SubProcesses'), mode='fortran')
    ...
    def write_c_bindings(self, writer):
        ...
        replace_dict['proc_number_to_position_map'] = ','.join(
            proc_number_to_position_map)

        # Now gather all possible combination of number of external legs
        all_externals_combination = set([])
        for me, number in processes:
            all_externals_combination.add(
                len(me.get('processes')[0].get('legs')))

        max_n_diags = 0
        for super_graph_list, ME in self.all_super_graphs:
            for sg in super_graph_list:
                max_n_diags = max(
                    max_n_diags, sg.call_signature['left_diagram_id'], sg.call_signature['right_diagram_id'])
        ...
        for i_proc, (super_graph_list, ME) in enumerate(self.all_super_graphs):
            ...
            for sg in super_graph_list:
                ...
                if len(mom_signs) > max_n_indep_mom:
                    logger.critical(
                        "The number of momenta sign in C_bindings.f is larger than the allowed maximum.")
                    logger.critical(
                        "The length is now adjusted so as to make sure C_bindings.f compiles, but this is indicative of a bug.")
        ...
        for n_external in sorted(list(all_externals_combination)):
            truncated_mom_list.append(
                "double complex P%d(0:3,%d)" % (n_external, n_external))
        replace_dict['truncated_mom_list'] = '\n'.join(truncated_mom_list)

        replace_dict['max_n_external'] = max(all_externals_combination)

        replace_dict['proc_positions_list'] = ','.join(
            '%d' % pos for pos in range(1, len(processes)+1))
        ...
        for i_proc, (me, proc_number) in enumerate(processes):
            ...
            n_incoming = len([l for l in me.get('processes')[
                             0].get('legs') if l.get('state') == False])
            matrix_element_call_dispatch.append(
                "P%d(I,%d)=DCMPLX(0.0d0,0.0d0)" % (n_external, n_external))
            matrix_element_call_dispatch.append("DO J=0,%d-1" % (n_external-1))
            matrix_element_call_dispatch.append(
                "P%d(I,J+1)=MOM_SIGNS(PROC_ID,SDL,SDR,J+1)*DCMPLX(P(J*8+2*I+1),P(J*8+2*I+2))" % n_external)
            matrix_element_call_dispatch.append(
                "IF ((J+1).le.%d) THEN" % n_incoming)
            matrix_element_call_dispatch.append("P%d(I,%d)=P%d(I,%d)+P%d(I,J+1)" % (
                n_external, n_external, n_external, n_external, n_external))
            ...
            for leg in me.get('processes')[0].get('legs'):
                ...
                SE_number = (abs(leg.get('id')) //
                             LTD_squared.self_energy_global_id_offset) % 10
                if SE_number != 0:
            ...
            if number_of_legs_for_each_SE:
                overall_phase *= (complex(0, -1)**(sum(v -
                                  3 for v in number_of_legs_for_each_SE.values())))
            matrix_element_call_dispatch.append(
                "ANS=ANS*DCMPLX(%d.0d0,%d.0d0)" % (int(overall_phase.real), int(overall_phase.imag)))
    ...
    def write_IO_bindings(self, writer, replace_dict):
        ...
        for i_proc, (me, proc_number) in enumerate(processes):
            ...
            matrix_element_call_dispatch.append("DO I=0,3")
            matrix_element_call_dispatch.append(
                "P%d(I,J)=DCMPLX(P(2*I),P(2*I+1))" % n_external)
            matrix_element_call_dispatch.append("ENDDO")
            matrix_element_call_dispatch.append("ENDDO")
            matrix_element_call_dispatch.append(
                'CALL M%d_SMATRIXHEL(P%d, -1, SDL, SDR, ANS)' % (i_proc, n_external))
    ...
    def convert_model(self, model, wanted_lorentz=[], wanted_couplings=[]):
        ...
        model_builder = alphaLoopModelConverter(
            model, write_dir, self.opt + self.proc_characteristic)
        ...
        if hasattr(self, 'aloha_model'):
            ...
            aloha_model = create_aloha.AbstractALOHAModel(
                os.path.basename(model.get('modelpath')))
    ...
    def generate_subprocess_directory(self, matrix_element, fortran_model, number):
        ...
        if 'use_physical_gluon_helicity_sum' not in self.alphaLoop_options:
            raise alphaLoopExporterError(
                "You probably forgot to specify '--mode=alphaloop' when running ./bin/mg5_aMC.")
        ...
        if 'prefix' in self.cmd_options:
            if self.cmd_options['prefix'] == 'int':
                ...
                proc_prefix = matrix_element.get(
                    'processes')[0].shell_string().split('_', 1)[1]
            else:
                raise Exception(
                    '--prefix options supports only \'int\' and \'proc\'')
        ...
        open(pjoin(dirpath, 'check_sa.f'), 'w').write(
            open(pjoin(plugin_src_path, 'Templates', 'check_sa.f'),
                 'r').read().replace('PROC_PREFIX_', proc_prefix)
        )
    ...
    def get_helicity_states(self, model, pdg, state, allow_reverse=True):
        ...
        if particle.get('spin') > 3:
            raise alphaLoopExporterError(
                "alphaLoop only support particles with spin 0, 1/2 or 1.")
    ...
    def write_matrix_element_v4(self, writer, matrix_element, fortran_model,
        ...
        replace_dict['ndiags_in_this_proc'] = len(
        ...
        if len(split_orders) == 0:
            ...
            # Extract JAMP lines
            jamp_lines = self.get_JAMP_lines(
                matrix_element, JAMP_format="JAMP(K,%s)")
        ...
        if len(split_orders) > 0:
            raise alphaLoopExporterError(
                "Split orders currently not supported by alphaLoop.")
        ...
        replace_dict['template_file'] = pjoin(
            MG5DIR, 'madgraph', 'iolibs', 'template_files', matrix_template)
        ...
        if write and writer:
            ...
            replace_dict['return_value'] = len(
                [call for call in helas_calls if call.find('#') != 0])
    ...
    def write_source_makefile(self, writer, *args, **opts):
        super(alphaLoopExporter, self).write_source_makefile(
            writer, *args, **opts)


class QGRAFExporter(object):

    qgraf_style = pjoin(plugin_src_path, 'Templates',
                        'qgraf', 'orientedGraphPython.sty')
    qgraf_root_path = pjoin(plugin_src_path, 'Templates', 'qgraf')
    qgraf_template_epem = pjoin(
        plugin_src_path, 'Templates', 'qgraf', 'qgraf_epem.dat')
    qgraf_template_no_s = pjoin(
        plugin_src_path, 'Templates', 'qgraf', 'qgraf_no_s.dat')
    qgraf_template_SE = pjoin(
        plugin_src_path, 'Templates', 'qgraf', 'qgraf_SE.dat')
    qgraf_template_amp = pjoin(
        plugin_src_path, 'Templates', 'qgraf', 'qgraf_amp.dat')
    ...
    qgraf_templates = {
        ...
        'amp': {'path': qgraf_template_amp, 'example': 'a a > a a'}
    }
    ...
    def __init__(self, process_definition, model, **opts):
        ...
        self.QGRAF_path = pjoin(
            plugin_path, os.path.pardir, 'libraries', 'QGRAF', 'qgraf')
        # TODO: Ensure no multiparticles in the initial states
        self.initial_states = [leg.get('ids')[0] for leg in process_definition.get(
            'legs') if not leg.get('state')]
        self.final_states = [leg.get('ids') for leg in process_definition.get(
            'legs') if leg.get('state')]
...
class HardCodedQGRAFExporter(QGRAFExporter):
    ...
    def __init__(self, process_definition, model, **opts):
        ...
        self.qgraf_model = pjoin(
            self.qgraf_root_path, '%s.dat' % self.alphaLoop_options['qgraf_model'])
        ...
        super(HardCodedQGRAFExporter, self).__init__(
            process_definition, model, **opts)
    ...
    def output(self, output_path):
        ...
        self.qgraf_output = pjoin(
            self.dir_path, 'qgraf', 'all_QG_supergraphs.py')
        ...
        logger.info(
            "Writing output of hardcoded QGRAF generation to '%s'" % self.dir_path)
        ...
        if self.alphaLoop_options['qgraf_cut_filter']:
            cuts = self.get_cuts(
                representative_proc, final_state_particle_ids=final_state_particle_ids+[None])
        else:
            cuts = None
        ...
        if self.alphaLoop_options['SG_name_list'] != []:
            ...
            super_graph_list = FORM_processing.FORMSuperGraphList(filter(
                lambda x: x[0].name in self.alphaLoop_options['SG_name_list'], super_graph_list))
            if len(super_graph_list) == 0:
                raise alphaLoopExporterError("None of {} SG names found!".format(
                    self.alphaLoop_options['SG_name_list']))
            logger.info("Working on {} of {} SuperGraphs.".format(
                len(super_graph_list), sg_number_checkpoint_1))
        ...
        if self.alphaLoop_options['checkpoint_lvl'] < 2 or not all(os.path.isfile(cp) for cp in self.checkpoint[1]):
            ...
            form_processor = FORM_processing.FORMProcessor(
                super_graph_list, computed_model, self.proc_def, force_options=FORM_processing.FORM_processing_options)
    ...
    def get_cuts(self, representative_process, final_state_particle_ids='any'):
        cuts = []
        additional_loops = len(
            representative_process['perturbation_couplings'])
    ...
    def build_qgraf_epem(self, representative_process):
        ...
        if self.alphaLoop_options['loop_induced']:
            virtual_loops = len(
                representative_process['perturbation_couplings']) + 2
        else:
            virtual_loops = len(
                representative_process['perturbation_couplings'])
        n_final_states = len([leg.get('id') for leg in representative_process.get(
            'legs') if leg.get('state') == True])
        ...
        required_final_states = [fields[0]
                                 for fields in self.final_states if len(fields) == 1]
        for field in set(required_final_states):
            if field >= 0:
                field_name = self.qgraf_field_replace[self.model.get_particle(abs(field))[
                    'name']]
            else:
                field_name = self.qgraf_field_replace[self.model.get_particle(abs(field))[
                    'antiname']]
            dict_replace['final_states'] += 'true=iprop[{},{},30];\n'.format(
                field_name, required_final_states.count(field))

        shutil.copy(self.qgraf_model, pjoin(
            self.dir_path, 'qgraf', 'model.dat'))
        shutil.copy(self.qgraf_style, pjoin(
            self.dir_path, 'qgraf', 'orientedGraphPython.sty'))
    ...
    def build_qgraf_no_s(self, representative_process, vetos=None, outgoing_state_suffix='', final_state_suffix='', qgraf_template=None):
        ...
        check_s_channel = representative_process.get(
            'required_s_channels') == []
        ...
        # with open(self.qgraf_template_epem if qgraf_template is None else qgraf_template, 'r') as stream:
        #     with open(qgraf_output, 'w') as f:
        #         f.write(stream.read() % dict_replace)
        ...
        dict_replace['outgoing_states'] += ", ".join(
            [("{}%s[{}]" % outgoing_state_suffix).format(*field) for field in incoming_fields])+";"
        ...
        for field in set(required_final_states):
            if field >= 0:
                field_name = self.qgraf_field_replace[self.model.get_particle(abs(field))[
                    'name']+final_state_suffix]
            else:
                field_name = self.qgraf_field_replace[self.model.get_particle(abs(field))[
                    'antiname']+final_state_suffix]
            dict_replace['final_states'] += 'true=iprop[{},{},30];\n'.format(
                field_name, required_final_states.count(field))
        ...
        with open(self.qgraf_template_no_s if qgraf_template is None else qgraf_template, 'r') as stream:
            with open(qgraf_output, 'w') as f:
                f.write(stream.read() % dict_replace)
    ...
    def build_qgraf_amp(self, representative_process, vetos=None, **opts):
        return self.build_qgraf_no_s(representative_process, vetos=vetos, outgoing_state_suffix='prime', final_state_suffix='', qgraf_template=self.qgraf_template_amp, **opts)

    def build_qgraf_SE(self, representative_process, vetos=None, **opts):
    ...
    def build_output_directory(self):
        ...
        Path(self.dir_path).mkdir(parents=True, exist_ok=True)
        Path(pjoin(self.dir_path, 'checkpoints')).mkdir(
            parents=True, exist_ok=True)
        Path(pjoin(self.dir_path, 'Rust_inputs')).mkdir(
            parents=True, exist_ok=True)
        Path(pjoin(self.dir_path, 'Cards')).mkdir(parents=True, exist_ok=True)
    ...
    def NON_HACKED_standalone_qgraf_file(self):
        logger.info('Create standalone QGRAF from: {}'.format(
            self.qgraf_output))
...
class LUScalarTopologyExporter(QGRAFExporter):
    ...
    def __init__(self, cli, output_path, topology, externals, name, lmb, model, edge_masses=None, benchmark_result=0.0, numerator='1', **opts):
        ...
        if edge_masses is not None:
            ...
            if len(self.unique_masses) > len(FORM_processing.dummy_scalar_PDGs):
                raise alphaLoopExporterError(
                    "Too many different edge masses have been specified.")
        ...
        for i_scalar in range(len(self.unique_masses)):
            ...
            if i_scalar != 0:
                new_dummy_mass_param = base_objects.ModelVariable(
                    FORM_processing.dummy_scalar_PDGs[337*10+i_scalar], '%.8e' % self.unique_masses[i_scalar], 'real', tuple([]))
                self.model.add_param(new_dummy_mass_param, tuple([]))
                self.model['parameter_dict'][FORM_processing.dummy_scalar_PDGs[337 *
                                                                               10+i_scalar]] = self.unique_masses[i_scalar]
    ...
    def build_output_directory(self):
        ...
        Path(pjoin(self.dir_path, 'lib')).mkdir(parents=True, exist_ok=True)
        Path(pjoin(self.dir_path, 'FORM')).mkdir(parents=True, exist_ok=True)
        Path(pjoin(self.dir_path, 'FORM', 'workspace')).mkdir(
            parents=True, exist_ok=True)
    ...
    def get_overall_factor(self):
        # there is an overall factor of -1 wrt Forcer, presumable due to the Wick rotation?
        return '-1*({})'.format(self.numerator)
    ...
    def output(self):
        ...
        process_definition = self.cli.extract_process(
            'e+ e- > a > d d~', proc_number=0)

        form_sg_list = self.get_sg_list(computed_model)

        form_processor = FORM_processing.FORMProcessor(
            form_sg_list, computed_model, process_definition, force_options=FORM_processing.FORM_processing_options)
...
class ScalarIntegralTopologyExporter(LUScalarTopologyExporter):
    ...
    def __init__(self, cli, output_path, topology, externals, default_kinematics, masses, name, lmb, model, benchmark_result=0.0,  numerator=None, **opts):
        ...
        if masses is not None:
            raise alphaLoopExporterError(
                "No support yet for masses in ScalarIntegralTopologyExporter.")
        self.masses = masses

        self.numerator = numerator

        super(ScalarIntegralTopologyExporter, self).__init__(cli, output_path, topology,
                                                             externals, name, lmb, model, benchmark_result=benchmark_result, **opts)
    ...
    def create_squared_topology(self):
        ...
        for e, nodes in edges_map.items():
            lone_nodes = [n for n in nodes if all(
                n not in other_nodes for e_i, other_nodes in edges_map.items() if e_i != e)]
            if len(lone_nodes) == 2:
                raise alphaLoopExporterError(
                    "Edge '%s' of the topology is completely disconnected." % e)
        ...
        for incoming_edge in self.externals[0]:
            if incoming_edge not in external_edges:
                raise alphaLoopExporterError(
                    "Specified external incoming edge '%s' could not be found." % incoming_edge)
            if external_edges[incoming_edge] != edges_map[incoming_edge][0]:
                raise alphaLoopExporterError(
                    "Incorrect direction for incoming external edge '%s'." % incoming_edge)
        ...
        for outgoing_edge in self.externals[1]:
            if outgoing_edge not in external_edges:
                raise alphaLoopExporterError(
                    "Specified external outgoing edge '%s' could not be found." % outgoing_edge)
            if external_edges[outgoing_edge] != edges_map[outgoing_edge][1]:
                raise alphaLoopExporterError(
                    "Incorrect direction for outgoing external edge '%s'." % outgoing_edge)
        ...
        squared_topology_info['squared_topology'] = [
            (e, edges_map[e][0], edges_map[e][1]) for e in internal_edges+list(self.externals[0])]
        ...
        for outgoing_edge in self.externals[1]:
            ...
            squared_topology_info['squared_topology'].append(
                (outgoing_edge, edges_map[outgoing_edge][0], effective_node_id))
        ...
        for _in_node in incoming_nodes:
            max_node_id += 1
            squared_topology_info['squared_topology'].append(
                ('q%i' % external_incoming_number, effective_node_id, max_node_id))
            external_incoming_number += 1

        # Combine the user's specified LMB with the LMB from the externals
        if self.lmb is not None:
            squared_topology_info['lmb'] = list(
                self.lmb) + list(self.externals[1])[:-1]
        else:
        ...
        squared_topology_info['default_kinematics'] = [
            [self.default_kinematics[incoming_edge]
                for incoming_edge in self.externals[0]],
            [self.default_kinematics[outgoing_edge]
                for outgoing_edge in list(self.externals[1])[:-1]],
        ]

        # We must divide the (-i)^N of the N-point effective mock-up scalar vertex on the right of the cutkosky cut.
        # extra minus one of unknown origin
        squared_topology_info['overall_factor'] = -1. / \
            (complex(0, -1.)**(len(self.externals[0])*2-2))
