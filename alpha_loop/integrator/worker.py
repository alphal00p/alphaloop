...
class Havana(object):
    ...
    def save_state(self, state_dump_name):
        ...
        class NoAliasDumper(yaml.SafeDumper):
            def ignore_aliases(self, data):
                return True
        base_dump_name = ('.'.join(state_dump_name.split(
            '.')[:-1]) if '.' in state_dump_name else state_dump_name)
        with open('%s.%s' % (base_dump_name, 'yaml'), 'w') as f:
            constructor_arguments = self.get_constructor_arguments(
                dump_format='bin', on_disk=True, file_name='%s_grids.yaml' % base_dump_name)
            constructor_arguments['integrands_constructor_args'] = [
                integrand.get_constructor_arguments() for integrand in self.integrands
            ]
            f.write(yaml.dump(constructor_arguments,
                    Dumper=NoAliasDumper, default_flow_style=False))
    ...
    def load_from_state(state_dump_name):
        ...
        if not os.path.exists('%s.yaml' % base_dump_name):
            raise HavanaIntegratorError(
                "Could not find state '%s.yaml' to load." % base_dump_name)
            ...
            if flat_record['on_disk']:
                ...
                for grid_file_path in flat_record['grid_files']:
                    with open(grid_file_path, 'rb') as f:
                        self.havana_grids.append(Havana.load_grid(
                            f.read(), seed=self.seed, format=grid_file_path.split('.')[-1]))
            else:
                self.havana_grids = []
                for serialized_grid in flat_record['grid_files']:
                    self.havana_grids.append(Havana.load_grid(
                        serialized_grid, seed=self.seed, format='bin'))
        else:
            ...
            for i_itg, integrand in enumerate(self.integrands):
                ...
                for phase in ['real', 'imag']:
                    if self.SG_ids is None:
                        a_grid = GridConstructor(cgc=ContinuousGridConstructor(
                            self.n_dimensions, self.n_bins, self.n_points_min))
                    elif self.n_channels_per_SG is None:
                        a_grid = GridConstructor(dgc=DiscreteGridConstructor(
                            [len(self.SG_ids),],
                            [
                                GridConstructor(cgc=ContinuousGridConstructor(
                                    integrand.n_dimensions_per_SG_id[SG_id], self.n_bins, self.n_points_min))
    ...
    def get_havana_grid_filenames(self, top_level_grid_filename, dump_format='bin'):
        ...
        for i_itg in range(0, len(self.havana_grids)//2):
            for i_phase, phase in enumerate(['real', 'imag']):
                ...
                grid_name = '.'.join(s for s in os.path.basename(
                    top_level_grid_filename).split('.')[:-1])
                grid_suffix = os.path.basename(
                    top_level_grid_filename).split('.')[-1]
    ...
    def get_flat_record(self, file_name=None, dump_format='bin', on_disk=True):
        ...
        havana_grid_file_paths = self.get_havana_grid_filenames(
            self.grid_file if file_name is None else file_name, dump_format=dump_format)
        if on_disk:
    ...
    def dump_grid(self, file_name=None, dump_format='bin'):
        ...
        flat_record = self.get_flat_record(
            file_name=file_name, dump_format=dump_format)
        ...
        with open(self.grid_file if file_name is None else file_name, 'w') as f:
            ...
            class NoAliasDumper(yaml.SafeDumper):
                def ignore_aliases(self, data):
                    return True
            f.write(yaml.dump(flat_record, Dumper=NoAliasDumper,
    ...
    def accumulate_results(self, results):
        ...
        raise NotImplementedError(
            "Currently the Havana wrapper only supports accumulating results by merging other havana grids.")
    ...
    def update_iteration_count(self, dump_grids=True):
        ...
        if len(self.bin_increase_factor_schedule) < self.n_iterations:
            ...
            bin_increase_factor = int(
                self.bin_increase_factor_schedule[self.n_iterations-1])
        self.n_bins = bin_increase_factor*self.n_bins
        for havana_grid in self.havana_grids:
            havana_grid.update(alpha=self.learning_rate, new_bin_length=self.n_bins, train_on_avg=(
                not self.optimize_on_variance))
    ...
    def get_grid_summary(self, sort_by_variance=True, show_channel_grid=True,
        ...
        for i_havana_grid, havana_grid in enumerate(self.havana_grids):
            ...
            if self.SG_ids is not None:
                ...
                pdfs = [cdfs[i]-(0. if i == 0 else cdfs[i-1])
            ...
            all_results.append(('%s[I%d%s]' % (
                'Re' if i_havana_grid % 2 == 0 else 'Im', i_havana_grid//2, itg_descr), integrand_result))

        # Place the reference result first
        all_results = [all_results[reference_result_index_here],]+[r for i_r,
                                                                   r in enumerate(all_results) if i_r != reference_result_index_here]
        ...
        def format_entry(r, short=False, show_p=False):
            str_res = '%s%-12s +- %-9.3g%-8s%s' % (
                bcolors.RED if (r['avg'] == 0. or abs(
                    r['err']/r['avg']) > 0.01) else bcolors.GREEN,
                '%s%.6g' % ('+' if r['avg'] >= 0. else '', r['avg']), r['err'],
                '(%.2g%%)' % (0. if r['avg'] ==
                              0. else abs(r['err']/r['avg'])*100.),
                bcolors.END
            )
            ...
            str_res += '|n0=%.1fM(%.2g%%)' % (
                ...
                0. if r['n_evals'] == 0 else (
                    r['n_zero_evals']/r['n_evals'])*100.
            )
            if show_p and r['p'] is not None:
                str_res += '|p=%.2g%%' % (r['p']*100.0)
            return str_res

        pt.add_column("Result", [entry['SG_name']
                      for entry in all_results[0][1]])
        pt.align["Result"] = 'l'

        for i_res, (integrand_name, integrand_results) in enumerate(all_results):
            pt.add_column(integrand_name, [format_entry(integrand_result, short=(all_results[0][1][i_row]['SG_name'] == 'Sum' or (
                i_res != 0 and not show_all_information_for_all_integrands)), show_p=(i_res == 0)) for i_row, integrand_result in enumerate(integrand_results)])
    ...
    def get_summary(self):
        ...
        avg, err, chi_sq, max_eval_negative, max_eval_positive, n_evals, n_zero_evals = self.havana_grids[
            self.reference_result_index].get_current_estimate()
        ...
        max_wgt_infl = 0. if self.n_points == 0 else max(
            abs(max_eval_negative), max_eval_positive)/(err*self.n_points)
        ...
        res.append("Result after %.1fM evaluations and %d iterations: %s%.6g +/- %.4g   (%.2g%%)%s, chi2=%s%.3g%s, max_wgt_infl=%s%.3g%s, zero_evals=%.3g%%" % (
            self.n_points/1.0e6, self.n_iterations, bcolors.RED if avg == 0. or abs(
                err/avg) > 0.01 else bcolors.GREEN, avg, err,
            0. if avg == 0. else (abs(err/avg))*100., bcolors.END,
            bcolors.RED if self.n_iterations == 0. or chi_sq/self.n_iterations > 5. else bcolors.GREEN, chi_sq /
            self.n_iterations if self.n_iterations > 0 else 0., bcolors.END,
            bcolors.RED if max_wgt_infl > 5. else bcolors.GREEN, max_wgt_infl, bcolors.END,
            (n_zero_evals/float(self.n_points)) *
            100. if self.n_points > 0 else 0.
        ))
        if self.target_result is not None:
            target_perc_diff = 0. if self.target_result == 0. else (
                (avg-self.target_result)/self.target_result)*100.0
            res.append(("%+{}s".format(len('Result after %.1fM evaluations and %d iterations' % (self.n_points/1.0e6, self.n_iterations)))) % ("vs target") +
                ...
                bcolors.END,
                bcolors.GREEN if abs(
                    target_perc_diff) <= 1.0 else bcolors.RED,
                target_perc_diff,
                bcolors.END
            ))
        for res_index, havana_grid in enumerate(self.havana_grids):
            avg, err, chi_sq, max_eval_negative, max_eval_positive, n_evals, n_zero_evals = havana_grid.get_current_estimate()
            max_wgt_infl = 0. if (self.n_points == 0 or err == 0.) else max(
                abs(max_eval_negative), max_eval_positive)/(err*self.n_points)
            if (self.integrand_descriptions is None) or (len(self.integrand_descriptions) <= (res_index//2)) or self.integrand_descriptions[res_index//2] == 'none':
                ...
                itg_descr = ('(%s%-{}s%s)'.format(max(len(itg_d) for itg_d in self.integrand_descriptions))
                             ) % (bcolors.GREEN, self.integrand_descriptions[res_index//2], bcolors.END)
            res.append('  %s %s[I%d%s] = %.5g +/- %.5g (%.2g%%), chi2=%.3g, max_wgt_infl=%.3g, zero_evals=%.3g%%' % (
                ...
                chi_sq, max_wgt_infl, (n_zero_evals/float(self.n_points)) *
                100. if self.n_points > 0 else 0.
            ))
...
class HavanaMockUp(object):
    ...
    def sample(self, batch_size):
        ...
        sample = numpy.random.rand(
            batch_size, 1+self.n_discrete_dimension+self.n_dimensions)
        for i_sample in range(batch_size):
            ...
            if self.discrete_grid is not None:
                ...
                sample[i_sample][1] = float(
                    self.discrete_grid[selected_SG]['SG_id'])
                ...
                if self.discrete_grid[selected_SG]['channel_grid'] is not None:
                    ...
                    havana_jacobian *= (
                        1./self.discrete_grid[selected_SG]['channel_grid'][selected_channel]['p'])
    ...
    def accumulate_from_havana_grid(self, grid):
        ...
        for index in range(0, self.n_integrands*2):
            ...
            self.current_integral_estimate[index] = self.wgt_sum[index] / \
                self.n_points
            self.current_error_estimate[index] = math.sqrt(
                ((self.wgt_sum_squared[index] / self.n_points) - self.current_integral_estimate[index]**2)/self.n_points)
            ...
            for SG_index in range(len(self.discrete_grid)):
                ...
                if self.discrete_grid[SG_index]['channel_grid'] is not None:
                    for channel_index in range(len(self.discrete_grid[SG_index]['channel_grid'])):
                        self.discrete_grid[SG_index]['channel_grid'][channel_index][
                            'n_points'] += grid.discrete_grid[SG_index]['channel_grid'][channel_index]['n_points']
                        self.discrete_grid[SG_index]['channel_grid'][channel_index][
                            'cum_wgt'] += grid.discrete_grid[SG_index]['channel_grid'][channel_index]['cum_wgt']
                        self.discrete_grid[SG_index]['channel_grid'][channel_index][
                            'cum_sqr_wgt'] += grid.discrete_grid[SG_index]['channel_grid'][channel_index]['cum_sqr_wgt']
    ...
    def accumulate_results(self, results):
        ...
        for index in range(0, self.n_integrands*2):
            self.wgt_sum[index] += float(sum(res[index] *
                                         res[self.n_integrands*2] for res in results))
            self.wgt_sum_squared[index] += float(
                sum((res[index]*res[self.n_integrands*2])**2 for res in results))
    ...
    def sync_grids(self, max_prob_ratio=None):
        ...
        self.sync_nested_grid(self.discrete_grid,
                              max_prob_ratio=max_prob_ratio)

        if self.discrete_grid is not None:
            for SG_bin in self.discrete_grid:
                self.sync_nested_grid(
                    SG_bin['channel_grid'], max_prob_ratio=max_prob_ratio)
    ...
    def update_iteration_count(self, dump_grids=True):
        ...
        if dump_grids:
            ...
            self.dump_grid()
            import shutil
            shutil.copy(self.grid_file, self.grid_file.replace(
                '.yaml', '_iteration_%d.yaml' % self.n_iterations))
    ...
    def sync_nested_grid(self, nested_grid, max_prob_ratio=None):
        ...
        for bin in nested_grid:
            if bin['n_points'] == 0:
                ...
                bin['error_estimate'] = math.sqrt(
                    ((bin['cum_sqr_wgt'] / bin['n_points']) - bin['integral_estimate']**2)/bin['n_points'])
        ...
        if max_p == 0.:
            ...
            for bin in nested_grid:
                bin['p'] = max(
                    bin['p'], max_p/max_prob_ratio) if max_prob_ratio is not None else bin['p']
                running_p_sum += bin['p']
    ...
    def get_grid_summary(self, sort_by_variance=True, show_channel_grid=True, **opts):
        ...
        sorted_bins = sorted([b for b in self.discrete_grid], key=lambda b: b['error_estimate']
                             if sort_by_variance else abs(b['integral_estimate']), reverse=True)
        ...
        for bins_chunk in [sorted_bins[i_chunk:i_chunk+5] for i_chunk in range(0, len(sorted_bins), 5)]:
            res.append('  '+' | '.join('%-45s' % (
                '%s(I=%.5g(%.2g%%),p=%.2f%%,n=%.2gM)' % (
                    ...
                    abs(bin['error_estimate']/bin['integral_estimate']) *
                    100. if bin['integral_estimate'] != 0. else 0.,
        ...
        if show_channel_grid:
            for SG_bin in sorted_bins:
                if SG_bin['channel_grid'] is not None:
                    res.append('Distribution of %d channels for SG %s:' % (
                        len(SG_bin['channel_grid']), SG_bin['SG_name']))
                    sorted_channel_bins = sorted([(i_c, b) for i_c, b in enumerate(
                        SG_bin['channel_grid'])], key=lambda b: b[1]['error_estimate'] if sort_by_variance else abs(b[1]['integral_estimate']), reverse=True)
    ...
    def get_summary(self):
        ...
        res = ["Result after %.1fM evaluations and %d iterations: %.5g +/- %.5g   (%.2g%%)" % (
            self.n_points /
            1.0e6, self.n_iterations, self.current_integral_estimate[
                self.reference_result_index], self.current_error_estimate[self.reference_result_index],
            0. if self.current_integral_estimate[self.reference_result_index] == 0. else
            (abs(self.current_error_estimate[self.reference_result_index] /
             self.current_integral_estimate[self.reference_result_index]))*100.
        )]
        if self.target_result is not None:
            target_perc_diff = 0. if self.target_result == 0. else (
                (self.current_integral_estimate[self.reference_result_index]-self.target_result)/self.target_result)*100.0
            res.append(("%+{}s".format(len('Result after %.1fM evaluations and %d iterations' % (self.n_points/1.0e6, self.n_iterations)))) % ("vs target") +
                       (": %s%-{}.5g del %-{}.5g%s %s(%-{}.2g%%)%s".format(
                           len('%.5g' %
                               self.current_integral_estimate[self.reference_result_index]),
                           len('%.5g  ' %
                               self.current_error_estimate[self.reference_result_index]),
                           len('%.2g' % (
                               0. if self.current_integral_estimate[self.reference_result_index] == 0. else
                               (abs(self.current_error_estimate[self.reference_result_index] /
                                    self.current_integral_estimate[self.reference_result_index]))*100.
        ...
        for res_index in range(self.n_integrands*2):
            res.append('  %s %s[I%d] = %.5g +/- %.5g (%.2g%%)' % (
                ...
                (abs(self.current_error_estimate[res_index] /
                 self.current_integral_estimate[res_index]))*100.
            ))
...
class ALStandaloneIntegrand(object):
    ...
    def __init__(self, n_integration_dimensions, alpha_loop_path, run_workspace, rust_input_folder,
        ...
        hyperparameters_path = os.path.join(
            self.run_workspace, run_dir, self.run_hyperparameters_filename)
        if not os.path.isfile(hyperparameters_path):
            raise WorkerException(
                "Could not find hyperparameter file at %s." % hyperparameters_path)
        ...
        try:
            ...
            raise WorkerException(
                "Failed to load the rust backend API in the Dask integrand.")
    ...
    def __call__(self, samples_batch, SG_id_per_sample=None, channel_id_per_sample=None):
        ...
        if self.frozen_momenta is not None:
            ...
            for i_sample, sample in enumerate(samples_batch):
                ...
                for i_v, v in enumerate([v[1:] for v in self.frozen_momenta['out']]):
                    xx, xy, xz, inv_jac = self.rust_worker.inv_parameterize(
                        list(v), n_loop_vs+i_v, self.E_cm**2)
        ...
        if self.n_dimensions_per_SG_id is not None and SG_id_per_sample is not None:
            for i_sample in range(len(samples_batch)):
                samples_batch[i_sample] = samples_batch[i_sample][:
                                                                  self.n_dimensions_per_SG_id[SG_id_per_sample[i_sample]]]
        ...
        if SG_id_per_sample is None and channel_id_per_sample is None:
            ...
            all_res = self.rust_worker.evaluate_integrand_batch(
                samples_batch, sg_ids=SG_id_per_sample)
        elif (SG_id_per_sample is not None) and (channel_id_per_sample is not None):
            all_res = self.rust_worker.evaluate_integrand_batch(
                samples_batch, sg_ids=SG_id_per_sample, channel_ids=channel_id_per_sample)
        else:
            raise WorkerException(
                "AlphaLoop integrand cannot sum over supergraphs but evaluate a single specific channel.")
...
def HavanaIntegrandWrapper(
    ...
    if not havana_mockup:
        ...
        all_res = numpy.ndarray(shape=(
            len(samples_batch), 2*len(integrands)+len(samples_batch[0])), dtype=float)
        for i_sample, sample in enumerate(samples_batch):
        ...
        if SG_ids_specified and channels_specified:
            ...
            raise Exception(
                "Havana cannot integrate specific integration channels while summing over all supergraphs.")

        for i_integrand, integrand in enumerate(integrands):

            # for _ in range(10):
            results = integrand(xs_batch, SG_id_per_sample=SG_id_per_sample,
                                channel_id_per_sample=channel_id_per_sample)
...
def run_job(input_payload, run_id, worker_id=-1, cache=None, output_path=None):
    ...
    print("Worker #%d processes a sample of %d points for job #%d..." % (
        worker_id, input_payload['n_points_to_sample'], call_options['job_id']))
    t0 = time.time()
    ...
    if input_payload['havana_mockup']:
        ...
        if havana_constructor_arguments['flat_record']['on_disk']:
            print("Worker #%d loads the havana sampler grid from files %s" % (
                worker_id, ', '.join(os.path.basename(
                    fp) for fp in havana_constructor_arguments['flat_record']['grid_files'])
            ))
        else:
            print("Worker #%d loads the havana sampler grid from serialized records using temporary file." % (
                worker_id))
        t0 = time.time()
        havana_constructor_arguments['flat_record']['n_points'] = 0
        if output_path is None:
            havana_constructor_arguments['grid_file'] = os.path.join(
                'run_%d_job_output_%d_grids.yaml' % (
                    run_id, input_payload['job_id'])
            )
        else:
            havana_constructor_arguments['grid_file'] = os.path.join(
        ...
        print("Worker #%d loaded the havana grids in %.5fs" %
              (worker_id, havana_loading_time))

    this_sample = havana_sampler.sample(input_payload['n_points_to_sample'])
    sample_generation_time = time.time()-t0
    print("Worker #%d completed sample generation in %.5fs." %
          (worker_id, sample_generation_time))
    call_options['samples_batch'] = this_sample
    call_options['havana_mockup'] = input_payload['havana_mockup']
    t0 = time.time()
    job_id, run_time, samples_evaluated = HavanaIntegrandWrapper(
        **call_options)
    print("Worker #%d evaluated all samples in %.5s." %
          (worker_id, time.time()-t0))
    ...
    if not input_payload['havana_mockup']:
        ...
        havana_updater_constructor_arguments = havana_updater.get_constructor_arguments(
            on_disk=(not (output_path is None)))
        ...
        if havana_updater_constructor_arguments['flat_record']['on_disk']:
            print("Worker #%d dumped Havana updater grids in %.5fs to files %s" % (worker_id,
                                                                                   ...
                                                                                   ', '.join(os.path.basename(
                                                                                       fp) for fp in havana_updater_constructor_arguments['flat_record']['grid_files'])
                                                                                   ))
        else:
            print("Worker #%d serialised results as a havana grid using a temporary local file." % (
                worker_id,))
        complete_result = {
...
def main(arg_tuple):
    ...
    input_path = os.path.join(
        run_workspace, 'run_%d_job_input_worker_%d.pkl' % (run_id, worker_id))
    input_path_done = os.path.join(
        run_workspace, 'run_%d_job_input_worker_%d.done' % (run_id, worker_id))
    output_path = os.path.join(
        run_workspace, 'run_%d_job_output_worker_%d.pkl' % (run_id, worker_id))
    output_path_done = os.path.join(
        run_workspace, 'run_%d_job_output_worker_%d.done' % (run_id, worker_id))
    while True:
        ...
        try:
            ...
            t_overall = time.time()
            print("Worker #%d received a new job at time t=%s" %
                  (worker_id, time.time()))
            sys.stdout.flush()
            t0 = time.time()
            input_payload = pickle.load(open(input_path, 'rb'))
            pickling_time = time.time() - t0
            print("Worker #%d unpickle job payload in %.5f s." %
                  (worker_id, pickling_time))
            output = run_job(input_payload, run_id, worker_id=worker_id,
            ...
            with open(output_path_done, 'w') as f:
                f.write("Marker file for job output done.")
            print("Worker #%d finished serialisation and dump of job output in %.5gs." % (
                worker_id, time.time()-t0))
            print("Worker #%d exact timing vs cumulative one: %.5fs vs %.5fs." % (
                worker_id, time.time()-t_overall, cumulative_measured_timing))
            print("Worker #%d completed job at time t=%s" %
                  (worker_id, time.time()))
            sys.stdout.flush()
            ...
            import traceback
            traceback.print_exc()
            print("The following exception was encountered in run #%d and worker #%d: %s" % (
                run_id, worker_id, str(e)))
            sys.stdout.flush()
            break
...
if __name__ == '__main__':
    ...
    if args.worker_id_min == args.worker_id_max:
        ...
        sys.stdout.flush()
        try:
            main(tuple([args.run_id, args.worker_id_min,
                 args.workspace_path, args.timeout]))
        except ImportError as e:
            # VHTOFIX
        ...
        try:
            with Pool(args.worker_id_max-args.worker_id_min+1) as p:
                p.map(main, [(args.run_id, worker_id, args.workspace_path, args.timeout)
                      for worker_id in range(args.worker_id_min, args.worker_id_max+1)])
        except Exception as e:
            print("Worker %d->%d finished (%s)." %
                  (args.worker_id_min, args.worker_id_max, str(e)))
