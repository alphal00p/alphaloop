...
alphaloop_basedir = os.path.abspath(os.path.join(os.path.dirname(
...
formatter = MyFormatter(fmt='%(name)-15s {}%(asctime)s{} %(message)s'.format(
    utils.bcolors.BLUE, utils.bcolors.END), datefmt='%Y-%m-%d,%H:%M:%S.%f')
...
def fast_remove(path, trashcan):
    ...
    if trashcan is None:
        ...
        trash_counter += 1
        shutil.move(path, pjoin(trashcan, '%d_%s' %
                    (trash_counter, os.path.basename(path))))
...
class AL_cluster(object):
    ...
    def __init__(self, architecture, n_workers, n_cores_per_worker, run_workspace, process_results_callback, run_id,
        ...
        if self.use_redis:
            ...
            if self.rq_path is None:
                raise HavanaIntegratorError(
                    "The executable 'rq' could not be found within the current PATH environment. Make sure to install it with pip install rq.")

        self.run_id = run_id
        self.current_status = None

        if self.architecture == 'local':
            self.work_monitoring_path = pjoin(
                self.run_workspace, 'run_%d_follow_workers' % self.run_id)
        ...
        if self.architecture not in self._SUPPORTED_CLUSTER_ARCHITECTURES:
            raise HavanaIntegratorError(
                "Cluster architecture not supported by ALcluster: %s" % architecture)
        ...
        if self.current_status is None or new_status != self.current_status:
            ...
            if self.monitor_callback is not None:
                cluster_status_str = 'Status of %d workers: %d pending, %d available, %s%d%s running. ' % (
                    new_status['PENDING']+new_status['FREE'] +
                    new_status['RUNNING'],
                ...
                if 'jobs' in new_status:
                    ...
                    cluster_status_str += '\nStatus of %d jobs: %s%d%s scheduled, %s%d%s submitted for this iteration, %s%d%s started, %s%d%s finished and not processed' % (
                        ...
                        bcolors.BLUE, len(
                            self.jobs_for_current_iteration), bcolors.END,
                    ...
                    if new_status['jobs']['deferred'] != 0:
                        cluster_status_str += ', %s%d%s deferred' % (
                            bcolors.BLUE, new_status['jobs']['deferred'], bcolors.END)
                    if new_status['jobs']['failed'] != 0:
                        cluster_status_str += ', %s%d%s failed for this iteration' % (
                            bcolors.RED, new_status['jobs']['failed'], bcolors.END)
                else:
                    cluster_status_str += 'A total of %d jobs are pending.' % (
                        self.n_jobs_in_queue)
    ...
    async def send_job(self, worker_id, payload):
        ...
        input_job_path = pjoin(
            self.run_workspace, 'run_%d_job_input_worker_%d.pkl' % (self.run_id, worker_id))
        input_job_path_done = pjoin(
            self.run_workspace, 'run_%d_job_input_worker_%d.done' % (self.run_id, worker_id))
        if os.path.exists(input_job_path):
            raise HavanaIntegratorError(
                "Attempted to send a job to a busy worker, this should never happen.")
        ...
        if 'job_id' in payload:
            self.jobs_for_current_iteration[payload['job_id']] = {
                'alpha_loop_job_id': payload['job_id'], 'sent_time': time.time()}
    ...
    def start_new_iteration(self):
        ...
        if self.use_redis:
            for job_id in self.jobs_for_current_iteration:
                try:
                    job_to_delete = rq.job.Job.fetch(
                        job_id, connection=self.redis_connection)
                    job_to_delete.delete()
                except Exception as e:
                    logger.warning(
                        "Failed to delete redis job %s. Exception: %s" % (job_id, str(e)))
            self.n_redis_job_failed_for_this_iteration = 0
        else:
        ...
        while self.active:
            ...
            try:
                ...
                if new_status['finished_and_not_processed'] > 0:
                    finished_jobs = rq.job.Job.fetch_many(
                        self.redis_queue.finished_job_registry.get_job_ids(), connection=self.redis_connection)
                    for finished_job in finished_jobs:
                        ...
                        if job_id in job_ids_already_handled:
                            logger.warning(
                                "The following job ID '%s' appeared on multiple occasions in the list of finished jobs, this should not happen." % str(job_id))
                            do_process_result = False
                        else:
                            finished_job = rq.job.Job.fetch(
                                job_id, connection=self.redis_connection)
                            if finished_job.get_status() != 'finished':
                                logger.warning("The following redis job '%s' was in the register of finished jobs but its status is '%s'." % (
                                    str(job_id), finished_job.get_status()))
                                do_process_result = False
                            else:
                                job_result = finished_job.result
                                if job_result is None:
                                    logger.warning(
                                        "The following redis job '%s' was in the register of finished jobs but its result is still None.")
                                    do_process_result = False
                ...
                if new_status['failed'] > 0:
                    failed_jobs = rq.job.Job.fetch_many(
                        self.redis_queue.failed_job_registry.get_job_ids(), connection=self.redis_connection)
                    for failed_job in failed_jobs:
                        ...
                        if job_id in job_ids_already_handled:
                            logger.warning(
                                "The following job ID '%s' appeared on multiple occasions in the list of failed jobs, this should not happen." % str(job_id))
                        else:
                            # Notify that this job failed
                            if job_id in self.jobs_for_current_iteration:
                        ...
                        try:
                            self.redis_queue.failed_job_registry.remove(
                                job_id, delete_job=True)
                        except Exception as e:
                            logger.warning("Failed to remove rq job %s, even though it should have been successful: %s." % (
                                str(job_id), str(e)))
                            pass
                        if job_id in self.jobs_for_current_iteration:
                            del self.jobs_for_current_iteration[job_id]
                ...
                logger.warning(
                    "Exception encountered when running redis_follow(), it will be ignored. Exception: %s" % str(e2))

    async def follow_workers(self):
        while self.active:
            try:
                if self.debug:
                    logger.info("Scanning for new job results...")
                io_files = [os.path.basename(f) for f in glob.glob(
                    pjoin(self.run_workspace, 'run_%d_job_output_worker_*.done' % self.run_id))]
                output_worker_ids = []
                for io_file in io_files:
                    output_match = re.match(
                        r'^run_(\d+)_job_output_worker_(?P<worker_id>\d+).done$', io_file)
                    if not output_match:
                        raise HavanaIntegratorError(
                            "Unexpected IO file found: %s" % io_file)
                ...
                if self.debug and len(output_worker_ids) > 0:
                    logger.info("Found new available results from workers: %s" % str(
                        output_worker_ids))
                for output_worker_id in output_worker_ids:
                    output_path = pjoin(self.run_workspace, 'run_%d_job_output_worker_%d.pkl' % (
                        self.run_id, output_worker_id))
                    output_path_done = pjoin(self.run_workspace, 'run_%d_job_output_worker_%d.done' % (
                        self.run_id, output_worker_id))
                    if self.debug:
                        logger.info(
                            "Deserialising the following job result: %s" % str(output_path))
                    job_result = pickle.load(open(output_path, 'rb'))
                    if self.debug:
                        logger.info("Done.")
                    input_path = pjoin(self.run_workspace, 'run_%d_job_input_worker_%d.pkl' % (
                        self.run_id, output_worker_id))
                    input_path_done = pjoin(self.run_workspace, 'run_%d_job_input_worker_%d.done' % (
                        self.run_id, output_worker_id))
                    if not os.path.exists(input_path):
                        raise HavanaIntegratorError(
                            "Matching input file for worker %d not found." % output_worker_id)
                    ...
                    if job_result != 'pong':
                        if job_result[0] in self.jobs_for_current_iteration:
                            if self.debug:
                                logger.info(
                                    "Processing results from worker: %d" % output_worker_id)
                            await self.process_results_callback(job_result)
                            if self.debug:
                                logger.info("Done.")
    ...
    async def deploy(self, wait_for_one_worker=True):
        ...
        if self.use_redis:
            redis_server_cmd = ['redis-server', '--port', '%d' % self.redis_port,
            ...
            if self.external_redis:
                ...
                while True:
                    try:
                        self.redis_connection = Redis(
                            host=self.redis_submitter_hostname, port=self.redis_port)
                        ...
                        if not warned:
                            warned = True
                            logger.info(
                                "You specified using an external instance of a redis server, but alphaLoop could not connect to any because of: %s." % str(e))
                ...
                redis_server_directory = pjoin(
                    self.run_workspace, 'redis_server')
                if os.path.exists(redis_server_directory):
                    shutil.rmtree(redis_server_directory)
                os.mkdir(redis_server_directory)
                # Start the redis server
                logger.info("Starting up a redis server in %s" %
                            redis_server_directory)
                redis_monitor_file = open(
                    pjoin(redis_server_directory, 'redis_server_live_log.txt'), 'a')
                self.redis_server_process = subprocess.Popen(
                    redis_server_cmd, cwd=redis_server_directory, stdout=redis_monitor_file, stderr=redis_monitor_file)
            ...
            if self.redis_queue_name is None:
                existing_queue_names = [
                    q.name for q in rq.Queue.all(self.redis_connection)]
                suffix = 0
            ...
            self.redis_queue = rq.Queue(
                this_queue_name, connection=self.redis_connection)
            ...
            for i_worker in range(self.n_workers):
                ...
                if self.use_redis:
                    cmd = [sys.executable, pjoin(alphaloop_basedir, 'alpha_loop', 'integrator', 'redis_worker.py'),
                           '--run_id', str(self.run_id), '--worker_id_min', str(
                        worker_id_min), '--worker_id_max', str(worker_id_max),
                        '--workspace_path', str(self.run_workspace), '--redis_server', 'redis://%s:%d' % (
                        self.redis_submitter_hostname, self.redis_port),
                        '--rq_path', self.rq_path, '--work_monitoring_path', (
                        'none' if not self.work_monitoring_path else self.work_monitoring_path),
                ...
                if self.worker_resources_path:
                    worker_env['LD_PRELOAD'] = pjoin(
                        self.worker_resources_path, 'libs', 'libscsdir.so')
                else:
                    worker_env['LD_PRELOAD'] = pjoin(
                ...
                self.all_worker_hooks.append(subprocess.Popen(
                    ...
                    stdout=(
                        self.worker_monitoring_file if self._FORWARD_WORKER_OUTPUT else subprocess.DEVNULL),
                    stderr=(
                        self.worker_monitoring_file if self._FORWARD_WORKER_OUTPUT else subprocess.DEVNULL),
                    env=worker_env
                ))
        ...
        if wait_for_one_worker:
            if self.use_redis:
                ...
                logger.info(
                    "Waiting for the redis parallelisation scheme to be properly setup...")
    ...
    def condor_deploy(self):
        ...
        if not os.path.exists(pjoin(self.run_workspace, 'run_%d_condor_logs' % self.run_id)):
            os.mkdir(pjoin(self.run_workspace,
                     'run_%d_condor_logs' % self.run_id))
        ...
        if self.worker_resources_path:
            ...
            if not os.path.exists(libscsdir_path):
                raise HavanaIntegratorError(
                    "Could not find libscsdir.so at '%s'. Make sure it is present and compiled." % libscsdir_path)

        if self.worker_resources_path:
            worker_run_workspace_dir = pjoin(
                self.worker_resources_path, 'ProcessOutput', 'run_workspace')
        else:
        ...
        with open(pjoin(self.run_workspace, 'run_%d_condor_submission.sub' % self.run_id), 'w') as f:
            ...
            if self.use_redis:
                format_dict['worker_script'] = pjoin(
                    alphaloop_basedir, 'alpha_loop', 'integrator', 'redis_worker.py')
    ...
    """ % format_dict)
                ...
                format_dict['worker_script'] = pjoin(
                    alphaloop_basedir, 'alpha_loop', 'integrator', 'worker.py')
                f.write(
                    """executable         = %(python)s
    ...
    """ % format_dict)
        ...
        submit_proc = subprocess.Popen(['condor_submit', pjoin(self.run_workspace, 'run_%d_condor_submission.sub' %
                                       self.run_id)], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        if submit_proc.returncode != 0:
            raise HavanaIntegratorError('Could not successfully submit condor jobs. Error:\n%s' % (
                submit_stderr.decode('utf-8')))
        else:
            submit_stdout_decoded = submit_stdout.decode('utf-8')
            matched_stdout = re.match(
            ...
            if matched_stdout:
                ...
                logger.info("A total of %d cluster workers were submitted to condor cluster id %d" % (
                    self.n_workers, cluster_id))
            else:
                raise HavanaIntegratorError(
                    'Could not interpret response from condor cluster:\n%s' % submit_stdout_decoded)

    def condor_kill(self):
        if len(self.all_worker_hooks) == 0:
            return
        try:
            kill_proc = subprocess.Popen(['condor_rm',]+[str(hook) for hook in self.all_worker_hooks],
            ...
            if kill_proc.returncode != 0:
                logger.warning('Could not successfully clean condor jobs. Error:\n%s' % (
                    kill_stderr.decode('utf-8')))
            else:
                logger.info('Successfully clean condor jobs:\n%s' %
                            (kill_stdout.decode('utf-8')))
        except Exception as e:
    ...
    async def submit_many_jobs(self, payloads):
        if not self.use_redis:
            raise HavanaIntegratorError(
                "Submitting many jobs at once is not supported nor useful when not using redis.")
    ...
    async def submit_job(self, payload, blocking=False):
        ...
        if blocking:
            ...
            if self.debug:
                logger.info(
                    "Adding a new job to the queue (now pending=%d)..." % self.n_jobs_in_queue)
    ...
    async def job_submitter(self):
        while self.active:
            try:
                ...
                if self.debug:
                    logger.info(
                        "Job submitter picked up a new job from the queue and it is now waiting for an available worker...")
                available_worker_id = await self.available_worker_ids.get()
                self.n_jobs_in_queue -= 1
                if self.debug:
                    logger.info("Job submitter found a free worker (#%d) to dispatch the job (now pending=%d)..." % (
                        available_worker_id, self.n_jobs_in_queue))
                await self.send_job(available_worker_id, payload)
                await self.update_status()
            except KeyboardInterrupt as e:
                break
        ...
        if self.architecture == 'local':
            ...
            if not self.keep:
                if os.path.exists(pjoin(self.run_workspace, 'run_%d_condor_logs' % self.run_id)):
                    shutil.rmtree(pjoin(self.run_workspace,
                                  'run_%d_condor_logs' % self.run_id))

                if os.path.exists(pjoin(self.run_workspace, 'run_%d_condor_submission.sub' % self.run_id)):
                    os.remove(pjoin(self.run_workspace,
                              'run_%d_condor_submission.sub' % self.run_id))
                if os.path.exists(pjoin(self.run_workspace, 'run_%d_condor_worker_arguments.txt' % self.run_id)):
                    os.remove(
                        pjoin(self.run_workspace, 'run_%d_condor_worker_arguments.txt' % self.run_id))
        ...
        if self.use_redis:
            ...
            try:
                ...
                for worker in workers:
                    try:
                        rq.command.send_shutdown_command(
                            self.redis_connection, worker.name)
                    except Exception as e:
                        pass
            except Exception as e:
                pass
...
class HavanaIntegrator(integrators.VirtualIntegrator):
    ...
    def __init__(self, integrands,
        ...
        # Setup a trashcan
        self.trashcan = pjoin(self.run_workspace,
                              'run_%s_trashcan' % self.run_id)
        ...
        if havana_bin_increase_factor_schedule is None:
            ...
            self.havana_bin_increase_factor_schedule = tuple(
                sum([[1, 1, 1, 1, 2],]*4, []))
        else:
    ...
    def update_status(self,
        ...
        monitoring_report.append('| Jobs completed: overall = %d (avg %s), and for this iteration = %d/%d/%d on %d cpus.\n| Total n_pts: %s%.1fM%s ( %s%s ms / pt%s on one core, %s %s pts / s%s overall ). n_pts for this iteration %s#%d: %.2fM/%.2fM (%.1f%%)%s.' % (
            n_jobs_total_completed, '%.3g min/job' % ((cumulative_job_time/n_jobs_total_completed) /
                                                      60.) if n_jobs_total_completed > 0 else 'N/A', n_done, n_submitted, n_jobs_for_this_iteration if n_jobs_for_this_iteration is not None else n_submitted,
            self.n_cpus, bcolors.GREEN, n_tot_points/(1.e6), bcolors.END, bcolors.GREEN, '%.3g' % (
                (curr_integration_time/n_tot_points)*self.n_cpus*1000.) if n_tot_points > 0 else 'N/A', bcolors.END,
            bcolors.GREEN, ('%.1fK' % (n_tot_points/curr_integration_time/1000.)
                            if n_tot_points > 0 else 'N/A'), bcolors.END, bcolors.BLUE,
            current_iteration_number,
            n_points_for_this_iteration/(1.e6), current_n_points/(1.e6), (float(
                n_points_for_this_iteration)/current_n_points)*100., bcolors.END
        ))
        ...
        parallelisation_efficiency = (
            cumulative_job_time / (wall_time*self.n_cpus))*100.
        ...
        if self.show_SG_grid:
            grid_summary = self.havana.get_grid_summary(
                ...
                show_channel_grid=(
                    self.MC_over_channels and self.show_channel_grid),
    ...
    async def process_job_result(self, job_result):
        ...
        try:
            if isinstance(job_result, int):
                if self._DEBUG:
                    logger.info(
                        "Job #%d failed and will thus be ignored." % job_result)
                self.n_jobs_awaiting_completion -= 1
                return

            job_ID, run_time, results = job_result
            if self._DEBUG:
                logger.info("Processing result from job #%d." % job_ID)
            ...
            if self._DEBUG:
                logger.info(
                    "Accumulating new results from job %d into havana..." % job_ID)
            ...
            if self._USE_HAVANA_MOCKUP:
                max_prob_ratio_for_this_iteration = max(
                    self.havana_max_prob_ratio / max(100. - 10.*(self.current_iteration-1), 1.), 3.)
                if self._DEBUG:
                    logger.info("Syncing Havana grids with the new results...")
                self.havana.sync_grids(
                    max_prob_ratio=max_prob_ratio_for_this_iteration)
                if self._DEBUG:
                    logger.info("Done updating Havana grids...")
                ...
                for index in range(0, len(self.integrands)):
                    ...
                    if (results['sample_statistics'][index]['max_eval_positive_xs'] is not None) and results['sample_statistics'][index]['max_eval_positive'] > self.integrands[index].max_eval_positive.value:
                        ...
                        self.integrands[index].max_eval_positive_xs[:
                                                                    ] = results['sample_statistics'][index]['max_eval_positive_xs']
                    if (results['sample_statistics'][index]['max_eval_negative_xs'] is not None) and results['sample_statistics'][index]['max_eval_negative'] < self.integrands[index].max_eval_negative.value:
                        ...
                        self.integrands[index].max_eval_negative_xs[:
                                                                    ] = results['sample_statistics'][index]['max_eval_negative_xs']
            ...
            if self._DEBUG:
                logger.info(
                    "Done processing new results from job %d into havana..." % job_ID)
    ...
    def handle_exception(self, loop, context):
        self.exit_now = True
        logger.critical(
            "The following exception happened in a coroutine:\n%s\nAborting integration now." % str(context))
    ...
    def prepare_standalone_directory(self):
        ...
        if os.path.isdir(self.standalone_workers) and os.path.isdir(pjoin(self.standalone_workers, 'ProcessOutput', 'run_workspace')):
            logger.info("Recycling existing resources directory '%s'" %
                        self.standalone_workers)
            run_workspace = pjoin(self.standalone_workers,
                                  'ProcessOutput', 'run_workspace')
            ...
            for itg in self.integrands:
                yaml_hyperparams = itg.get_constructor_arguments()[
                    'run_hyperparameters_filename']
                shutil.copy(
                    pjoin(itg_args['run_workspace'],
                          itg_args['run_dir'], yaml_hyperparams),
            ...
            standalone_dir = pjoin(
                self.standalone_workers, 'alphaLoop_%s' % str(uuid.uuid4()))
        ...
        logger.info(
            "Copying relevant process output resources into '%s'..." % standalone_dir)
        process_output_path = pjoin(standalone_dir, 'ProcessOutput')
        os.mkdir(process_output_path)
        shutil.copytree(itg_args['rust_input_folder'],
                        pjoin(process_output_path, 'Rust_inputs'))
        shutil.copy(
            itg_args['cross_section_set_file_path'],
            pjoin(process_output_path, 'Rust_inputs', os.path.basename(
                itg_args['cross_section_set_file_path']))
        )
        shutil.copytree(os.path.join(itg_args['rust_input_folder'], os.path.pardir, 'lib'), pjoin(
            process_output_path, 'lib'))
        ...
        shutil.copy(pjoin(itg_args['alpha_loop_path'], 'ltd.so'), pjoin(
            alphaLoop_path, 'ltd.so'))
        ...
        for ext in ['so', 'dylib']:
            libscsdir_path = pjoin(
                itg_args['alpha_loop_path'], 'libraries', 'scs', 'out', 'libscsdir.%s' % ext)
            if os.path.exists(libscsdir_path):
                shutil.copy(libscsdir_path, pjoin(
                    libs_path, 'libscsdir.%s' % ext))
                found_it = True
                break
        if not found_it:
            raise HavanaIntegratorError("Could not find libscsdir.so at '%s'. Make sure it is present and compiled." % (
                pjoin(itg_args['alpha_loop_path'], 'libraries', 'scs', 'out', 'libscsdir.[so|dylib]')))
    ...
    async def async_integrate(self):
        ...
        n_dimensions = len(
            self.integrands[0].dimensions.get_continuous_dimensions())
        if not all(len(integrand.dimensions.get_continuous_dimensions()) == n_dimensions for integrand in self.integrands):
            raise HavanaIntegratorError(
                "In Havana implementations, all integrands must have the same dimension.")

        if not self.MC_over_SGs and self.MC_over_channels:
            raise HavanaIntegratorError(
                "Havana cannot perform a Monte-Carlo over channels but not over supergraphs.")

        if not self.MC_over_SGs:
            SG_ids = None
        else:
            if self.selected_SGs is None:
                SG_ids = [(i_SG, SG['name']) for i_SG, SG in enumerate(
                    self.cross_section_set['topologies'])]
            else:
                SG_ids = []
                ...
                if len(SG_ids) != len(self.selected_SGs) or len(SG_ids) == 0:
                    raise HavanaIntegratorError(
                        "Not all specified SG names were found in the cross_section set.")
        ...
        if self.standalone_workers:
            ...
            if self.cluster_type == 'condor':
                ...
                self.worker_resources_path = pjoin(
                    '.', os.path.basename(resources_dir_name))
            else:
                # Locally, we keep resources at the original location
                self.worker_resources_path = os.path.abspath(
                    resources_dir_name)
            ...
            for a in integrands_constructor_args:
                a['alpha_loop_path'] = pjoin(
                    self.worker_resources_path, 'alpha_loop')
                a['run_workspace'] = pjoin(
                    self.worker_resources_path, 'ProcessOutput', 'run_workspace')
                a['cross_section_set_file_path'] = pjoin(
                    self.worker_resources_path, 'ProcessOutput', 'Rust_inputs', os.path.basename(a['cross_section_set_file_path']))
                a['rust_input_folder'] = pjoin(
                    self.worker_resources_path, 'ProcessOutput', 'Rust_inputs')

        logger.info("Staring alphaLoop integration with Havana and cluster '%s' as run #%d" % (
            self.cluster_type, self.run_id))
        logger.info("Number of SG considered: %s (%s)" % (
            len(self.cross_section_set['topologies']) if SG_ids is None else len(
                SG_ids), "Monte-Carlo'ed over" if self.MC_over_SGs else 'explicitly summed per sample'
        ))
        ...
        logger.info("%s%s%s" %
                    (bcolors.GREEN, "Lean back and enjoy...", bcolors.END))
        ...
        try:
            ...
            logger.info(
                "Now deploying cluster '%s' and waiting for at least one job to become active..." % self.cluster_type)
            ...
            while True:
                ...
                n_jobs_for_this_iteration = math.ceil(
                    current_n_points/float(self.batch_size))
                self.n_points_for_this_iteration = 0
                ...
                sampling_grid_constructor_arguments = self.havana.get_constructor_arguments(
                    on_disk=write_common_grid_inputs_to_disk)

                n_submitted = 0
                n_done = 0
                last_n_done = 0

                logger.info("Now running iteration %s#%d%s" %
                            (bcolors.GREEN, self.current_iteration, bcolors.END))
                ...
                while True:
                    ...
                    if n_remaining_points > 0 and self.n_jobs_awaiting_completion < self.n_cpus+max(int(self.n_cpus/20.), 2):
                        ...
                        this_n_points = min(
                            n_remaining_points, self.batch_size)
                        ...
                        havana_grid_constructor_arguments = dict(
                            sampling_grid_constructor_arguments)
                        if self.seed:
                            havana_grid_constructor_arguments['seed'] = int(
                                self.seed+100000*job_ID)
                        else:
                        ...
                        if self.use_redis and self.bulk_redis_enqueuing > 0:
                            ...
                            if len(job_payloads) >= self.bulk_redis_enqueuing:
                                if self._DEBUG:
                                    logger.info("Submitting batch of %d jobs." % (
                                        len(job_payloads)))
                                await self.al_cluster.submit_many_jobs(job_payloads)
                                job_payloads.clear()
                                if self._DEBUG:
                                    logger.info("Done with submission of batch of %s jobs." % (
                                        len(job_payloads)))
                        else:
                            if self._DEBUG:
                                logger.info("Submitting job %d." % job_ID)
                            ...
                            if self._DEBUG:
                                logger.info(
                                    "Done with submission of job %d." % job_ID)
                        ...
                        if show_waiting:
                            ...
                            if self._DEBUG:
                                logger.info(
                                    "Now waiting for jobs to complete and freeing workers...")
                ...
                trash_counter += 1
                shutil.move(self.trashcan, '%s_%d_%s' %
                            (self.trashcan, trash_counter, 'being_removed'))
                os.mkdir(self.trashcan)
                subprocess.Popen('rm -rf %s_%d_%s' % (self.trashcan, trash_counter,
                                 'being_removed'), shell=True, stdout=subprocess.PIPE)
                if self.exit_now:
                    break

                t2 = time.time()
                if self._USE_HAVANA_MOCKUP and self._DEBUG:
                    logger.info(
                        "Updating Havana grids for iteration #%d..." % self.current_iteration)
                self.havana.update_iteration_count(
                    dump_grids=self.dump_havana_grids)

                if not self._USE_HAVANA_MOCKUP:
                    self.havana.save_state(
                        pjoin(self.run_workspace, 'latest_results.yaml'))
                    for i_itg, integrand in enumerate(self.integrands):
                        grid_index = (i_itg*2 if self.phase ==
                                      'real' else (i_itg*2 + 1))
                        ...
                        integrand.max_eval_positive_xs[:] = [
                            0.,]*len(integrand.max_eval_positive_xs)
                        integrand.max_eval_negative.value = max_eval_negative
                        # TODO propagate this info too
                        integrand.max_eval_negative_xs[:] = [
                            0.,]*len(integrand.max_eval_negative_xs)

                if self._USE_HAVANA_MOCKUP and self._DEBUG:
                    logger.info("Done updating Havana grids in %.3g ms." %
                                ((time.time()-t2)*1000.))
                self.cumulative_processing_time += time.time() - t2

                if self.n_iterations is not None and self.current_iteration >= self.n_iterations:
                    logger.info("Max number of iterations %d reached." %
                                self.n_iterations)
                    break

                if self.n_max is not None and self.n_tot_points >= self.n_max:
                    logger.info(
                        "Max number of sample points %d reached." % self.n_max)
                    break
    ...
    def integrate(self):
        ...
        if self.cluster_type in ['local', 'condor']:
            ...
            raise HavanaIntegratorError(
                "Cluster architecture %s not supported." % self.cluster_type)
