...
def to_right_diag(left_diag):
    ...
    new_edge_ids = list(str(x) for x in sorted(list(range(1, len(left_diag['edges'])+1))))
    old_edge_ids =sorted( list((left_diag['edges']).keys()))
    for i,ne in enumerate(new_edge_ids):
        for nn in (left_diag['nodes']).values():
            nn['edge_ids']=tuple(map(lambda x: ne if x==old_edge_ids[i] else x, nn['edge_ids']))
    for i, ee in enumerate(list(left_diag['edges'])):
        left_diag['edges'][new_edge_ids[i]] = left_diag['edges'].pop(old_edge_ids[i])
    for nn in (left_diag['nodes']).values():
...
def sew_amp_diags(ld, rd):
        ...
        for nn in sewed_graph['nodes'].values():
            if isinstance(nn['momenta'], str):
                ...
                nn['momenta'] = tuple(x.replace(mom, rhs)
                                      for x in nn['momenta'])
        sewed_graph['analytic_num']=sewed_graph['analytic_num'].replace(mom,rhs)
    ...
    for i, nn in enumerate(list(sewed_graph['nodes'])):
        sewed_graph['nodes'][new_vert[i]] = sewed_graph['nodes'].pop(old_vert[i])
...
if True:
    ...
    import ggHHHLeftDiags as ggHHHld


left_diags = ggHHHld.graphs
...
# for a,b in sew_amp_diags(left_diags[1],right_diags[0])['edges'].items():
#     print((a, b['momentum']))
...
save_dict_to_file(super_graphs,'ggHHHsgs')
