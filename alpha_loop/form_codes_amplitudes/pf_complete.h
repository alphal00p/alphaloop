...
#do i = 0,{`LOOPS'-1}
	...
	id energyk`i'^r?*ncmd(?z,z1?,0) = sum_(cLTDs,0, r - nargs_(?z) + 0, a(r,cLTDs,?z)*z1^cLTDs);
	B+ a;
	.sort:collect-a;
	keep brackets;

	repeat id a(r?,cLTDs?,?z,z1?) = -sum_(energyk, cLTDs+1,r-nargs_(?z) + 0, a(r,energyk,?z)*z1^(energyk-cLTDs-1));
	id a(r?,cLTDs?) = delta_(r,cLTDs);
	.sort:energy-k`i';
#enddo
id num=1;
