...
try:
    ...
    raise BaseException(
        "Install yaml python module in order to import/export topologies from/to yaml.")
...
FORM_processing_options = {
    ...
    'uv_test': None,
    'allow_external_momenta_isomorphic_mapping': True,
...
def temporary_preprocess_multiline_blocks(FORM_output):
    ...
    def combine_lines(lines, split_terms=False):
        ...
        if not split_terms:
            ...
            for i_line, line in enumerate(lines[1:]):
                if not (line.startswith('       - ') or line.startswith('       + ')):
                    if current_running_line == '':
                        combined_lines[-1] = combine_lines(
                            [combined_lines[-1], line], split_terms=False)[0]
                        continue
                    else:
                        current_running_line = combine_lines(
                            [current_running_line, line], split_terms=False)[0]
                else:
                    current_running_line = combine_lines(
                        [current_running_line, line], split_terms=False)[0]
                ...
                if len(combined_lines) == 1:
                    if balanced_parenthesis(combined_lines[0]) is None:
                        raise FormProcessingError(
                            "No imbalanced parenthesis in first line to combine within an output C context.")

                b_par = balanced_parenthesis(current_running_line)
                if b_par is None:
                    combined_lines.append('      _ += %s' %
                                          current_running_line)
                    current_running_line = ''
                else:
                    if current_running_line[b_par] == ')':
                        if i_line != len(lines)-2:
                            raise FormProcessingError(
                                'The closing parenthesis of the an output C block did not happen on the last line of the block.')
                        combined_lines.append(
                            '      _ += %s' % current_running_line)
                        current_running_line = ''
                    else:
                        continue
            ...
            if current_running_line != '':
                raise FormProcessingError(
                    'Accumulated line of a C output block was not added:\n%s' % current_running_line)
    ...
    for i_line, line in enumerate(FORM_output.split('\n')):
        ...
        if stripped_line == ';':
            if not is_in_multiline_block:
                if len(lines_in_current_block) != 0:
                    raise FormProcessingError(
                        'ERROR A in preprocessing of multiline blocks of FORM output at line %d, line:\n%s' % (i_line, line))
                new_lines[-1] += ';'
                continue
            if len(lines_in_current_block) == 0:
                raise FormProcessingError(
                    'ERROR B in preprocessing of multiline blocks of FORM output at line %d, line:\n%s' % (i_line, line))
            new_lines.extend(combine_lines(
                lines_in_current_block, split_terms=False))
            lines_in_current_block = []
            continue
        ...
        if stripped_line.startswith('_ +='):
            if not is_in_multiline_block:
                raise FormProcessingError(
                    'ERROR C in preprocessing of multiline blocks of FORM output at line %d, line:\n%s' % (i_line, line))
            if len(lines_in_current_block) != 0:
                raise FormProcessingError(
                    'ERROR D in preprocessing of multiline blocks of FORM output at line %d, line:\n%s' % (i_line, line))
        ...
        if stripped_line[-1] == ';':
            new_lines.extend(combine_lines(
                lines_in_current_block, split_terms=True))
            lines_in_current_block = []
            is_in_multiline_block = False

    if is_in_multiline_block or len(lines_in_current_block) > 0:
        raise FormProcessingError(
            'ERROR E in preprocessing of multiline blocks of FORM output at line %d, line:\n%s' % (i_line, line))
    ...
    def process_line(l, first=False):
        ...
        for structure, var in list(known_structures.items()):
            if var is None and structure in processed_line:
                ...
                new_lines.append('%s = %s;' %
                                 (known_structures[structure], structure))
            if structure in processed_line:
                processed_line = processed_line.replace(
                    structure, known_structures[structure])
        ...
        for power_arg, power_int in list(re.findall(power_re, l)):
            ...
            if power_structure not in structures_seen:
                ...
                new_lines.append('%s = %s;' % (
                    structures_seen[power_structure], power_structure))
            processed_line = processed_line.replace(
                power_structure, structures_seen[power_structure])
                ...
                for factors_pair in chunks(factors, 2):
                    if len(factors_pair) == 2:
                        if factors_pair not in structures_seen:
                            ...
                            new_lines.append('%s = %s;' % (
                                structures_seen[factors_pair], '*'.join(factors_pair)))
        ...
        if currently_in_unbalanced_context is None:
            ...
            if unbalanced_index is None:
                ...
                if not line.strip().startswith('return'):
                    raise FormProcessingError(
                        "Unbalanced parenthesis for a line that is not a return line, this is not expected. Line #%d: '%s'" % (i_line+1, line))
                ...
                if line[unbalanced_index] != '(' or line[unbalanced_index-1] != '*':
                    raise FormProcessingError(
                        "Unexpected strucuture for line with unbalanced parenthesis. Line: '%s'" % line)
                processed_output.append(
                    line[:(unbalanced_index-1)].replace('return', 'Z1_ = ')+';')
                processed_output.extend(process_line(
                    line[(unbalanced_index+1):], first=True))
        else:
            if not line.startswith('      _ +='):
                # if not (line.startswith('       +') or line.startswith('       -')):
                raise FormProcessingError(
                    "Unexpected format of line in unbalanced context. Line: '%s'" % line)
            unbalanced_index = balanced_parenthesis(line)
            if unbalanced_index is None:
                ...
                if line[-1] != ';' or unbalanced_index != (len(line)-2):
                    raise FormProcessingError(
                        "Unexpected end of balanced context. Line: '%s'" % line)
                currently_in_unbalanced_context = None
    ...
    def get_mathematica_rendering_code(self, model, FORM_id=None, lmb_id=None):
        ...
        for edge_key, edge_data in self.edges.items():
            ...
            edge_label_pieces = ['psi' if edge_data['PDG']
                                 in dummy_scalar_PDGs else get_part_name(edge_data['PDG']),]
...
""" % self.get_mathematica_rendering_code(model, FORM_id=FORM_id, lmb_id=lmb_id)
    ...
    def complexity_score_lmb(self, lmb, model, edge_key_to_name, edge_name_to_power, edge_name_to_pdg, weight_combination_rule='product'):
        ...
        topo_generator, _, _ = self.get_topo_generator(
            specified_LMB=lmb_edge_names)

        # And adjust all signatures (incl. the string momenta assignment accordingly)
        lmb_edge_names = [edge_name.replace('q', 'p') if edge_name.startswith(
            'q') else edge_name[1:] for edge_name in lmb_edge_names]
        ...
        edge_name_to_power = {edge_name.replace('q', 'p') if edge_name.startswith(
            'q') else edge_name[1:]: v for edge_name, v in edge_name_to_power.items()}
        ...
        for edge_key in sorted(self.edges.keys()):
            ...
            if all(s == 0 for s in signatures[self.edges[edge_key]['name']][0]):
                ...
                continue
            sig = list(signatures[self.edges[edge_key]['name']][0]) + \
                list(signatures[self.edges[edge_key]['name']][1])
            ...
            if particle.get('spin') == 2:
                edge_weights.append(
                    sum([1 if abs(wgt) != 0 else 0 for wgt in sig]))
            else:
                edge_weights.append(1)
        ...
        for node_key in sorted(self.nodes.keys()):
            ...
            if not found_a_loop_momentum:
                ...
                continue
            vertex_spins = tuple(sorted([model.get_particle(abs(pdg)).get(
                'spin') for pdg in self.nodes[node_key]['PDGs']]))
            if vertex_spins in [(3, 3, 3), (1, 3, 3), (1, 1, 3)]:
                ...
                for edge_key in self.nodes[node_key]['edge_ids']:
                    sig = list(signatures[self.edges[edge_key]['name']][0]) + \
                        list(signatures[self.edges[edge_key]['name']][1])
                    node_weight += sum([1 if abs(wgt) !=
                                       0 else 0 for wgt in sig])
                ...
                raise FormProcessingError("No rule for assigning complexity weight for vertex with spins %s when computing LMB complexitiy score." % (
                    ','.join('%d' % s for s in vertex_spins)))

        product_score = functools.reduce(
            lambda a, b: a*b, edge_weights)*functools.reduce(lambda a, b: a*b, node_weights)
        sum_squared_score = sum(
            wgt**2 for wgt in edge_weights) + sum(wgt**2 for wgt in node_weights)
        final_score = None
        ...
        if weight_combination_rule == 'product':
            final_score = (raised_powers_in_LMB, -
                           product_score, -sum_squared_score)
        elif weight_combination_rule == 'sum_squared':
            final_score = (raised_powers_in_LMB, -
                           sum_squared_score, -product_score)
        elif weight_combination_rule == 'combination':
            final_score = (raised_powers_in_LMB, -product_score -
                           sum_squared_score, -product_score, -sum_squared_score)
        else:
            raise FormProcessingError(
    ...
    def adjust_LMBs(self, model):
        ...
        for edge_name, sig in sig_map.items():
            ...
            for s in [a, b]:
                if s in sig_powers:
                    ...
                    sig_powers[s] = 1
        edge_name_to_power = {e_name: sig_powers[(tuple(sig_map[e_name][0]), tuple(
            sig_map[e_name][1]))] for e_name in edge_name_to_key}
        ...
        edge_name_to_pdg = {edge['name']: edge['PDG']
                            for edge in self.edges.values()}
        # add the prefix p
        edge_name_to_pdg = {'p%s' % k: v for k,
                            v in edge_name_to_pdg.items() if not k.startswith('p')}
        ...
        if FORM_processing_options['optimise_generation_lmb']:
            ...
            for (lmb_index, lmb) in enumerate(all_lmbs):
                score = self.score_lmb(
                    lmb, model, edge_key_to_name, edge_name_to_power, edge_name_to_pdg)
        ...
        if isinstance(FORM_processing_options['reference_lmb'], int):
            ...
            reference_lmb = 1
            all_lmbs = [tuple([edge_name_to_key[e]
                              for e in FORM_processing_options['reference_lmb'][self.name]]), ]
        ...
        if (reference_lmb is not None) or (forced_LMB_index is not None):
            ...
            if not (isinstance(FORM_processing_options['reference_lmb'], dict) and self.name in FORM_processing_options['reference_lmb']):
                original_LMB = tuple(sorted(original_LMB, key=lambda e_key: abs(
                    edge_name_to_pdg[edge_key_to_name[e_key]])))
            ...
            signatures = {edge_name_to_key[edge_name]: [sig[0],
                                                        [i+o for i, o in zip(
                                                            sig[1][:len(sig[1])//2], sig[1][len(sig[1])//2:])]
                                                        ] for edge_name, sig in signatures.items()}
            for edge_key, edge_data in self.edges.items():
                # Fix for QGRAF pipeline
                if not isinstance(edge_key, tuple):
    ...
    def generate_ltd_integrand_verbose(self, g, graph_id, constants, unique_propagators, prop_id, prop_mom_in_lmb, energies, on_shell_condition):
        ...
        for co in [x for css in g.ltd_cut_structure for x in
            ...
            if mat == []:
                ...
                # the transpose matrix is used for signatures
                nmi = np.linalg.inv(np.array(mat).transpose())
                cb_to_lmb = np.linalg.inv(np.array(mat))

            cb_to_lmb_str = []
            ltdenergy = ['ltd{0},{1}E{0}'.format(
                prop_id[(li, pi)], '+' if cut_sign == 1 else '-') for (cut_sign, (li, pi)) in co]
            for i, r in enumerate(cb_to_lmb):
                mm = []
                for (c, (_, (li, pi))) in zip(r, co):
                    if c != 0:
                        ext = self.momenta_decomposition_to_string(
                            (prop_mom_in_lmb[prop_id[(li, pi)]][1], prop_mom_in_lmb[prop_id[(li, pi)]][2]), True)
                        if ext != '':
                            ext = '{}energies({})'.format(
                                '-' if c == 1 else '+', ext)

                        mm.append('{}ltd{}'.format(
                            '' if c == 1 else '-', prop_id[(li, pi)], ext))
            ...
            for li, l in enumerate(g.loop_lines):
                ...
                for (sig_sign, (cut_sign, (lci, pci))) in zip(sig_map, co):
                    if sig_sign != 0:
                        sig = tuple(list(prop_mom_in_lmb[prop_id[(
                            lci, pci)]][1]) + list(prop_mom_in_lmb[prop_id[(lci, pci)]][2]))
                        minsig = tuple(
                            list(-prop_mom_in_lmb[prop_id[(lci, pci)]][1]) + list(-prop_mom_in_lmb[prop_id[(lci, pci)]][2]))
                        ...
                        if sig in on_shell_condition:
                            ...
                            momp = self.momenta_decomposition_to_string((prop_mom_in_lmb[prop_id[(
                                lci, pci)]][1], prop_mom_in_lmb[prop_id[(lci, pci)]][2]), True)
                            momp = '0' if momp == '' else '+energies({})'.format(
                                momp)

                        energy.append('{},ltd{},{}{}'.format(
                            int(sig_sign), prop_id[(lci, pci)], '+' if -sig_sign == 1 else '-', momp))
                        # the full energy including the cut sign
                        energy_full += '{}E{}{}{}'.format('+' if int(cut_sign * sig_sign) == 1 else '-', prop_id[(
                            lci, pci)], '+' if -int(sig_sign) == 1 else '-', momp)
                ...
                for pi, p in enumerate(l.propagators):
                    ...
                    if (1, (li, pi)) in co:
                        ...
                        if prop_plus not in unique_propagators:
                            unique_propagators[prop_plus] = len(
                                unique_propagators)
                        propagators.append('invd{},{}'.format(
                            unique_propagators[prop_plus], prop_plus))
                        r.append(
                            'prop(1/({0}),1,ltd{1},0,E{1}){2}'.format(prop_plus, prop_id[(li, pi)], powmod))
                        if p.power > 1:
                            # add derivative prescription
                            der.append('ltd{},{}'.format(
                                prop_id[(li, pi)], p.power - 1))
                    elif (-1, (li, pi)) in co:
                        prop_min = '-2*E{0}'.format(prop_id[(li, pi)])
                        if prop_min not in unique_propagators:
                            unique_propagators[prop_min] = len(
                                unique_propagators)
                        propagators.append('invd{},{}'.format(
                            unique_propagators[prop_min], prop_min))
                        r.append(
                            'prop(1/({0}),-1,ltd{1},0,-E{1}){2}'.format(prop_min, prop_id[(li, pi)], powmod))
                        if p.power > 1:
                            der.append('ltd{},{}'.format(
                                prop_id[(li, pi)], p.power - 1))
                    else:
                        sig = tuple(list(prop_mom_in_lmb[prop_id[(
                            li, pi)]][1]) + list(prop_mom_in_lmb[prop_id[(li, pi)]][2]))
                        minsig = tuple(
                            list(-prop_mom_in_lmb[prop_id[(li, pi)]][1]) + list(-prop_mom_in_lmb[prop_id[(li, pi)]][2]))
                        ...
                        if sig in on_shell_condition:
                            ...
                            momp = self.momenta_decomposition_to_string(
                                (prop_mom_in_lmb[prop_id[(li, pi)]][1], prop_mom_in_lmb[prop_id[(li, pi)]][2]), True)
                            momp = '' if momp == '' else '+energies({})'.format(
                                momp)

                        prop_plus = '{}+E{}{}'.format(energy_full,
                                                      prop_id[(li, pi)], momp)
                        prop_min = '{}-E{}{}'.format(energy_full,
                                                     prop_id[(li, pi)], momp)
            ...
            diagres.append('{}*{}{}{}({})'.format(
                ltd_closure_factor,
                'ltdcbtolmb({})*'.format(','.join(cb_to_lmb_str)
                                         ) if len(cb_to_lmb_str) > 0 else '',
    ...
    def generate_ltd_integrand(self, g, graph_id, constants, unique_propagators, prop_id, prop_mom_in_lmb, energies, on_shell_condition):
        ...
        for li, l in enumerate(g.loop_lines):
            ...
            for pi, p in enumerate(l.propagators):
                ...
                mom = '+'.join('{}ltd{}'.format('' if s == 1 else ('-' if s == -1 else '{}*'.format(s)), i + 1)
                               for i, s in enumerate(l.signature) if s != 0)

                res.append('prop({}{},E{}){}'.format(
                    mom, momp, prop_id[(li, pi)], powmod))

        prefactor = '' if g.n_loops == 0 else '(-1)^{0}*(2*pi*i_)^{0}*'.format(
            g.n_loops)
        ...
        for cut in topo.cuts:
                ...
                for uv_structure in diag_info['uv']:
                    ...
                    for uv_subgraph in uv_structure['uv_subgraphs']:
                        ...
                        for dg in uv_subgraph['derived_graphs']:
                            ...
                            graphs.append(
                                (signature_offset, dg['id'], dg['loop_topo'], None))
                            if 'soft_ct_id' in dg:
                                massct = (
                                    False, uv_subgraph['onshell']) if uv_subgraph['onshell'] is not None else None
                                graphs.append(
                                    (signature_offset, dg['soft_ct_id'], dg['loop_topo_orig_mass'], massct))
                            if 'onshell_ct_id' in dg:
                                massct = (
                                    True, uv_subgraph['onshell']) if uv_subgraph['onshell'] is not None else None
                                graphs.append(
                                    (signature_offset, dg['onshell_ct_id'], dg['loop_topo_orig_mass'], massct))
                        graphs.append(
                            (signature_offset, uv_subgraph['integrated_ct_id'], uv_subgraph['integrated_ct_bubble_graph'], None))
                        signature_offset += uv_subgraph['derived_graphs'][0]['loop_topo'].n_loops
                    else:
                        graphs.append(
                            (signature_offset, uv_structure['remaining_graph_id'], uv_structure['remaining_graph_loop_topo'], None))
            ...
            for signature_offset, graph_id, g, onshell in graphs:
                ...
                if onshell is not None:
                    ...
                    ext_edge = next(ee for ee in self.edges.values(
                    ) if ee['name'] == self_energy_ext_edges[0])
                ...
                for li, l in enumerate(g.loop_lines):
                    ...
                    for pi, p in enumerate(l.propagators):
                        ...
                        if onshell is None:
                            ...
                            ext_edge_sigs = [next(ee for ee in self.edges.values() if ee['name'] == e)[
                                'signature'] for e in self_energy_ext_edges]
                            shift = np.array([0]*topo.topo.n_loops)
                            ...
                            extshift = np.array(
                                list(extshift) + [0]*len(extshift))
                            # drop the spatial shift
                            totalmom = self.momenta_decomposition_to_string(
                                (lmp, np.array([0]*len(ext_edge_sigs[0][1]))), True)

                        # recycle energy computations when there are duplicate edges
                        edge_mass = 'masses({})'.format(
                            next(ee for ee in self.edges.values() if ee['name'] == p.name)['PDG'])
                        energy_instr = '{},{}'.format(
                            totalmom, edge_mass if not p.uv else 'mUV')
                        if not is_constant:
                            if energy_instr not in unique_energies:
                                unique_energies[energy_instr] = len(
                                    unique_energies)

                            pf_prefactor.append('2*E{}'.format(unique_energies[energy_instr]) if p.power == 1 else '(2*E{})^{}'.format(
                                unique_energies[energy_instr], p.power))
                            e_str = 'E{},{}'.format(
                                unique_energies[energy_instr], energy_instr)
                            if e_str not in energies:
                                energies.append(e_str)
                            prop_id[(li, pi)] = unique_energies[energy_instr]
                            prop_mom_in_lmb[unique_energies[energy_instr]] = (
                                lmp, shift, extshift)

                            for _ in range(p.power):
                                shift_map.append(list(shift) + list(extshift))
                                unique_energy.append(
                                    unique_energies[energy_instr])
                        else:
                            for _ in range(p.power):
                ...
                if integrand_type == "both" or integrand_type == "PF":
                    ...
                    if len(signatures) == 0:
                        ...
                        res, used_props = pf.to_FORM(
                            energy_index_map=unique_energy, den_library=den_library, on_shell_conditions=on_shell_condition)
                        res = '\n'.join(['\t' + l for l in res.split('\n')])
                        resden = ','.join('invd{},{}'.format(
                            i, d) for i, d in enumerate(den_library) if i in used_props)
...
""".format(len(unique_pf), ltd_integrand_body))
    ...
    def get_node_scaling(self, pdgs_input, names_of_edges_connected=None):
        ...
        pdgs = tuple([(-(abs(pdg) % 1000) if pdg < 0 else pdg % 1000)
                     for pdg in pdgs_input if pdg not in [1122,]])
    ...
    def generate_squared_topology_files(self, root_output_path, model, process_definition, n_jets, numerator_call, final_state_particle_ids=(), jet_ids=None, write_yaml=True, bar=None,
        ...
        particle_masses = {e['name']: model['parameter_dict'][model.get_particle(
            e['PDG']).get('mass')].real for e in self.edges.values()}
        ...
        topo = LTD.squared_topologies.SquaredTopologyGenerator(edge_map_lin,
                                                               ...
                                                               loop_momenta_names=tuple(
                                                                   [l for l, s in loop_momenta]),
                                                               loop_momenta_signs=tuple(
                                                                   [s for l, s in loop_momenta]),
                                                               ...
                                                               numerator_structure={},
                                                               FORM_integrand={'call_signature': {
                                                                   'id': call_signature_ID}},
                                                               edge_weights={e['name']: self.get_edge_scaling(
                                                                   e['PDG'], e['name']) for e in self.edges.values()},
                                                               vertex_weights={nv: self.get_node_scaling(n['PDGs'], tuple(sorted(
                                                                   [self.edges[eID]['name'] for eID in n['edge_ids']]))) for nv, n in self.nodes.items()},
        ...
        self.generate_integrand(topo, integrand_type,
                                workspace, call_signature_ID, progress_bar=bar)
    ...
    def generate_form_input(self, topo):
        ...
        for cut_index, cut in enumerate(topo.cuts):
            ...
            for diag_info in cut['diagram_info']:
                ...
                for uv_index, uv_structure in enumerate(diag_info['uv']):
                    ...
                    for lmb_index, (r, aff, extshift, _, _, _) in enumerate(zip(*uv_structure['forest_to_cb_matrix'])):
                        if all(x == 0 for x in r):
                            ...
                            continue
                        mom = ''.join('{}fmb{}'.format(sign_prefix(a), forest_index + 1)
                                      for forest_index, a in enumerate(r) if a != 0)
                        ...
                        for cmb_index, a in enumerate(aff):
                            ...
                            d = self.momenta_decomposition_to_string(
                                ([0] * n_loops, cut['cuts'][cmb_index]['signature'][1]), False, a, True)
                            shift += '{}c{}{}'.format(sign_prefix(-a),
                                                      cmb_index + 1, d)

                        shift += self.momenta_decomposition_to_string(
                            ([0] * n_loops, extshift), True, -1, True)
                        m = 'c{},{}{}'.format(
                            lmb_index + cmb_offset + 1, strip_plus(mom), shift)
                        forest_to_cb.append(m)
                    if len(forest_to_cb) > 0:
                        forest_element.append(
                            'cbtofmb({})'.format(','.join(forest_to_cb)))
                    ...
                    for fmb_index, (_, _, _, r, aff, extshift) in enumerate(zip(*uv_structure['forest_to_cb_matrix'])):
                        if all(x == 0 for x in r):
                            assert (all(x == 0 for x in aff))
                            continue
                        mom = ''.join('{}cs{}'.format(sign_prefix(
                            a), cmb_index + cmb_offset + 1) for cmb_index, a in enumerate(r) if a != 0)
                        ...
                        for cmb_index, a in enumerate(aff):
                            ...
                            d = self.momenta_decomposition_to_string(
                                ([0] * n_loops, cut['cuts'][cmb_index]['signature'][1]), False, -a, True).replace('p', 'ps')
                            shift += '{}cs{}{}'.format(sign_prefix(a),
                                                       cmb_index + 1, d)

                        shift += self.momenta_decomposition_to_string(
                            ([0] * n_loops, extshift), True, 1, True).replace('p', 'ps')
                        m = 'fmbs{},{}{}'.format(
                            fmb_index + 1, strip_plus(mom), shift)
                        cb_to_forest.append(m)
                    if len(cb_to_forest) > 0:
                        forest_element.append(
                            'fmbtocb({})'.format(','.join(cb_to_forest)))

                    diag_moms = ','.join(self.momenta_decomposition_to_string(
                        lmm, False) for lmm in uv_structure['remaining_graph_loop_topo'].loop_momentum_map)

                    if diag_moms != '':
                        forest_element.append('{}diag({},{},{})'.format(
                            bubble_uv_derivative, cut['id'], topo.topo_map[uv_structure['remaining_graph_id']], diag_moms))
                    else:
                        forest_element.append('{}diag({},{})'.format(
                            bubble_uv_derivative, cut['id'], topo.topo_map[uv_structure['remaining_graph_id']]))

                    if uv_index == 0:
                        if diag_moms != '':
                            diag_set_uv_conf.append(
                                'forestid({},{})'.format(len(uv_forest), diag_moms))
                        else:
                            diag_set_uv_conf.append(
                                'forestid({})'.format(len(uv_forest)))
                    ...
                    for uv_subgraph in uv_structure['uv_subgraphs']:
                        ...
                        # all derived graphs have the same topo
                        uv_loop_graph = uv_subgraph['derived_graphs'][0]['loop_topo']
                        uv_diag_moms = ','.join(self.momenta_decomposition_to_string(
                            lmm, False) for lmm in uv_loop_graph.loop_momentum_map)
                        vertex_structure = []
                        subgraph_vertices = set(
                            v for ll in uv_loop_graph.loop_lines for v in (ll.start_node, ll.end_node))
                            ...
                            for ll in uv_loop_graph.loop_lines:
                                for (dv, outgoing) in ((ll.start_node, 1), (ll.end_node, -1)):
                                    ...
                                    for s, lmm in zip(ll.signature, uv_loop_graph.loop_momentum_map):
                                        if s != 0:
                                            loop_mom_sig += self.momenta_decomposition_to_string(
                                                lmm, False, s * outgoing, True)
                        ...
                        for i, (ll_sig, propagators, _raised_power) in enumerate(uv_loop_graph.uv_loop_lines[0]):
                            ...
                            for s, lmm, lm_shift in zip(ll_sig, uv_loop_graph.loop_momentum_map, uv_loop_graph.uv_loop_lines[1]):
                                if s != 0:
                                    loop_mom_sig += self.momenta_decomposition_to_string(
                                        lmm, False, s, True)
                                    loop_mom_shift += self.momenta_decomposition_to_string(
                                        lm_shift, False, s, True)
                            ...
                            for (edge_name, param_shift, power) in propagators:
                                    ...
                                    for _ in range(power):
                                        if loop_mom_shift == '':
                                            uv_props.append('uvprop({},t{},0,{})'.format(
                                                strip_plus(loop_mom_sig), i, edge_mass))
                                        else:
                                            uv_props.append('uvprop({},t{},{},{})'.format(strip_plus(
                                                loop_mom_sig), i, strip_plus(loop_mom_shift), edge_mass))
                                    continue
                                ...
                                for (ext_index, s) in enumerate(param_shift[1]):
                                    if s != 0:
                                        ...
                                        sig_l, sig_ext = np.array(ext_edge['signature'][0], dtype=int), np.array(
                                            ext_edge['signature'][1], dtype=int)
                                        if ext_mom_sig == '':
                                            ext_mom_sig = (
                                                s * sig_l, s * sig_ext)
                                        else:
                                            ext_mom_sig = (
                                                ext_mom_sig[0] + s * sig_l, ext_mom_sig[1] + s * sig_ext)

                                ext_mom_sig = (([x for x in ext_mom_sig[0]]), ([
                                               x for x in ext_mom_sig[1]]))
                                ext_mom_sig = self.momenta_decomposition_to_string(
                                    ext_mom_sig, False, 1, True)

                                for _ in range(power):
                                    uv_props.append('uvprop({},t{},{},{})'.format(
                                        loop_mom_sig, i, strip_plus(loop_mom_shift + ext_mom_sig), edge_mass))
                        ...
                        if uv_diag_moms == '':
                            ...
                            uv_diag = 'uvtopo({})'.format(
                                uv_subgraph['first_occurrence_id'])
                        else:
                            uv_diag = 'uvtopo({},{})'.format(
                                uv_subgraph['first_occurrence_id'], uv_diag_moms)

                        if uv_subgraph['onshell'] is not None and FORM_processing_options['on_shell_renormalisation']:
                            ext_edge_sig = next(ee for ee in self.edges.values(
                            ) if ee['name'] == uv_subgraph['onshell'][0])
                            totalmom = self.momenta_decomposition_to_string(
                                ext_edge_sig['signature'], False)
                            uv_diag += '*onshell({},masses({}))'.format(
                                totalmom, ext_edge_sig['PDG'])

                        if FORM_processing_options['generate_integrated_UV_CTs']:
                            uv_diag += '*intuv(1 - {}*diag({},{},{}))'.format('*'.join(vertex_structure),
                                                                              cut['id'], topo.topo_map[uv_subgraph['first_occurrence_id'] + 1], uv_diag_moms)

                        uv_conf_diag = '-tmax^{}*{}*{}'.format(
                            uv_subgraph['taylor_order'], '*'.join(uv_props), uv_diag)
                        if uv_conf_diag not in uv_diagrams:
                            uv_diagrams.append(uv_conf_diag)

                        uv_conf = 'uvdiag({})'.format(
                            uv_diagrams.index(uv_conf_diag))

                        sg_call = 'subgraph({}{},{},{})'.format(uv_subgraph['graph_index'],
                                                                (',' if len(uv_subgraph['subgraph_indices']) > 0 else '') + ','.join(
                                                                    str(si) for si in uv_subgraph['subgraph_indices']),
                ...
                # add the amplitude loop count to the cmb start
                cmb_offset += len(diag_info['uv'][0]
                                  ['forest_to_cb_matrix'][0][0])
                uv_forest.append('+\n\t'.join(conf))
            ...
            diag_momenta = '' if diag_momenta == [
            ] else '*' + '*'.join(diag_momenta)
            tder = '*tder({})'.format(','.join(str(c['power']) for c in cut['cuts'] if c['power'] > 1)) if any(
                c['power'] > 1 for c in cut['cuts']) else ''
            conf = 'conf({},{},{},{}){}{}'.format(
                cut['id'], cut_index, cmb_map, conf, tder, diag_momenta)
            if diag_set_uv_conf != []:
        ...
        self.configurations = (topo_map, uv_diagrams,
                               uv_forest, ' +'.join(configurations))
...
class FORMSuperGraphIsomorphicList(list):
    ...
    def generate_numerator_functions(self, model, additional_overall_factor='', output_format='c', workspace=None, FORM_vars=None, active_graph=None, process_definition=None, recycle=False, forced_options=None):
        ...
        # set the order
        if FORM_processing_options['generate_integrated_UV_CTs']:
            FORM_vars['MAXPOLE'] = len(characteristic_super_graph.edges) - len(characteristic_super_graph.nodes) + 1 - min(
                len(c['cuts']) - 1 for c in characteristic_super_graph.squared_topology.cuts)
        else:
            FORM_vars['MAXPOLE'] = 0
        ...
        if workspace is None:
            ...
            FORM_source = pjoin(selected_workspace, '%s.frm' %
                                FORM_source_to_run)
        ...
        with open(pjoin(selected_workspace, 'input_%d.h' % i_graph), 'w') as f:
            ...
            f.write('{}\n\n'.format('\n'.join('Fill forest({}) = {};'.format(
                i, uv) for i, uv in enumerate(uv_forest))))
            f.write('L CONF =\n +{};\n\n'.format(conf))
            f.write('L F = {}\n;'.format(form_input))

        if platform.system() != 'Darwin':
            form_settings = formset.generate_form_settings(
                percentage=FORM_processing_options["FORM_use_max_mem_fraction"] * 100., ncpus=FORM_processing_options["FORM_parallel_cores"] * FORM_processing_options["cores"])
        else:
            form_settings = {}
        ...
        if FORM_processing_options["FORM_parallel_cores"] == 1:
            ...
            FORM_cmd = [FORM_processing_options["tFORM_path"],
                        '-w{}'.format(FORM_processing_options["FORM_parallel_cores"]), '-W']
        ...
        if recycle and os.path.isfile(pjoin(selected_workspace, 'FORM_run_cmd_%d.exe' % i_graph)) and \
            ...
            logger.info(
                "Recycling already existing FORM output for graph #%d. Remove file out_integrand_*_%d.proto_c to force a reprocessing." % (i_graph, i_graph))
            characteristic_super_graph.code_generation_statistics = self.analyze_FORM_output(
                open(pjoin(selected_workspace, 'numerator_%d.log' % i_graph), 'r').read(), FORM_vars)
            return True
        ...
        # or not os.path.isfile(pjoin(selected_workspace,'out_integrand_%d.proto_c'%i_graph)):
        if r.returncode != 0:
    ...
    def multiplicity_factor(self, iso_id, workspace, form_source):
        ...
        if r.returncode != 0:
            # raise FormProcessingError(
            logger.critical(
                "FORM processing for the computation of the isocheck between the follow supergraphs" +
                " %s failed with error:\n%s\nCommand:\n%s" % (
                    ', '.join(g.name for g in self), r.stdout.decode(
                        'UTF-8'), 'cd %s; %s' % (workspace, cmd)
                )
            )
            return (self, -0.1337)
...
class FORMSuperGraphList(list):
    ...
    def from_dict(cls, dict_file_path, first=None, merge_isomorphic_graphs=False, verbose=False, model=None, workspace=None, cuts=None):
        ...
        for i, g in enumerate(m.graphs):
            if isinstance(list(g['nodes'].keys())[0], str):
                ...
                for edge_key, edge in g['edges'].items():
                    edge['vertices'] = (
                        node_names[edge_key[0]], node_names[edge_key[1]])
                ...
                for e_key, e in g['edges'].items():
                    ...
                    e['momentum'] = FORMSuperGraph.momenta_decomposition_to_string(
                        smap[e['name']], set_outgoing_equal_to_incoming=True)
                ...
                for n_key, n in g['nodes'].items():
                    ...
                    for idx, eid in zip(n['indices'], n['edge_ids']):
                        ...
                        shifts = [
                            sgn * x for x in smap[g['edges'][eid]['name']][1]]
                        new_moms += [FORMSuperGraph.momenta_decomposition_to_string(
                            [signature, shifts], set_outgoing_equal_to_incoming=True)]
                    # print("\t\t",n['momenta'])
        ...
        for g in full_graph_list:
            ...
            all_signatures = {}
            for e_key, e in g.edges.items():
                pos_signature = tuple(tuple(s for s in sp)
                                      for sp in e['signature'])
                neg_signature = tuple(tuple(-s for s in sp)
                                      for sp in e['signature'])
            ...
            for sig, edges in all_signatures.items():
                for edge_key, sign in edges:
                    ...
                    g.edges[edge_key]['signature'] = [[-s for s in sp]
                                                      for sp in g.edges[edge_key]['signature']]
        ...
        iso_groups = []
        n_externals = max(len([1 for e in graph.edges.values(
        ) if e['type'] == 'in' or e['type'] == 'out']) for graph in full_graph_list)
        if model is None:
            pdg_primes = {pdg: sp.prime(i + n_externals + 1) for i, pdg in enumerate(
                [1, 2, 3, 4, 5, 6, 11, 12, 13, 21, 22, 25, 82])}
        else:
            pdg_primes = {pdg: sp.prime(
                i + n_externals + 1) for i, pdg in enumerate([p['pdg_code'] for p in model['particles']])}
            ...
            for g_id, graph in enumerate(full_graph_list):
                ...
                for e in graph.edges.values():
                    undirected_edges.add(
                        (tuple(sorted(e['vertices'])), (('type', e['type']), ('name', e['name']))))
                ...
                for ue, attrs in undirected_edges:
                    ...
                    for e in graph.edges.values():
                        if tuple(sorted(e['vertices'])) == ue:
                            ...
                            if not FORM_processing_options['allow_external_momenta_isomorphic_mapping']:
                                if e['type'] in ['in', 'out']:
                                    ext_id += 1
                ...
                for (ref_graph, ref_colors), graph_list in iso_groups:
                    v_maps = ref_graph.get_isomorphisms_vf2(
                        g, edge_color1=ref_colors, edge_color2=edge_colors)
                    if v_maps != [] and merge_isomorphic_graphs:
                        ...
                        out_edges = [
                            ee for ee in graph_list[0].edges.values() if ee['type'] == 'out']
                        ...
                        for out_edge in out_edges:
                            ...
                            if mapped_out_edge_type == 'in':
                                if is_external_flow_flipped is None:
                                    ...
                                    raise FormProcessingError(
                                        "Inconsistent mapping of incoming and outgoing externals during graph isomorphism checks. (1)")
                            elif mapped_out_edge_type == 'out':
                                if is_external_flow_flipped is None:
                                    is_external_flow_flipped = False
                                elif is_external_flow_flipped:
                                    raise FormProcessingError(
                                        "Inconsistent mapping of incoming and outgoing externals during graph isomorphism checks. (2)")
                            else:
                                raise FormProcessingError(
                                    "Inconsistent mapping of incoming and outgoing externals during graph isomorphism checks. (3)")
                        if is_external_flow_flipped is None:
                            raise FormProcessingError(
                                "Inconsistent mapping of incoming and outgoing externals during graph isomorphism checks. (4)")
                        ...
                        shifts = []
                        selected_edges = []
                        n_loops = len(graph_list[0].edges) - \
                            len(graph_list[0].nodes) + 1
                        for loop_var in range(n_loops):
                            ...
                            orig_edge_in_ref = next(ee for ee in ref_graph.es if tuple(
                                sorted(orig_edge['vertices'])) == (ee.source + 1, ee.target + 1))
                            ...
                            new_edge = sorted(
                                new_edges, key=lambda x: len(x['momentum']))[0]
                            ...
                            if model.get_particle(abs(new_edge['PDG']))['self_antipart']:
                                ...
                                match_direction = v_map[new_edge['vertices']
                                                        [0]-1]+1 == orig_edge['vertices'][0]
                        ...
                        for e in graph.edges.values():
                            ...
                            e['momentum'] = graph.momenta_decomposition_to_string(
                                e['signature'], set_outgoing_equal_to_incoming=False)

                        if verbose:
                            parser = re.compile(r'(^|\+|-)(k|p)(\d*)')
                            n_incoming = sum(
                                [1 for edge in graph.edges.values() if edge['type'] == 'in'])
                            ...
                            for momentum in old_momenta:
                                parsed = [e.groups()
                                          for e in parser.finditer(momentum)]
                                signature = ([0 for _ in range(n_loops)], [
                                             0 for _ in range(n_incoming)])
                                for pa in parsed:
                                    if pa[1] == 'p':
        ...
        if not cuts is None:
            if FORM_processing_options["cores"] == 1:
                ...
                pool = multiprocessing.Pool(
                    processes=FORM_processing_options["cores"])
        ...
        if workspace is not None:
            ...
            shutil.copy(pjoin(plugin_path, "multiplicity.frm"),
                        pjoin(selected_workspace, 'multiplicity.frm'))
            shutil.copy(pjoin(plugin_path, "numerator.frm"),
                        pjoin(selected_workspace, 'numerator.frm'))
            shutil.copy(pjoin(plugin_path, "raw_numerator.frm"),
                        pjoin(selected_workspace, 'raw_numerator.frm'))
            shutil.copy(pjoin(plugin_path, "tensorreduce.frm"),
                        pjoin(selected_workspace, 'tensorreduce.frm'))
            shutil.copy(pjoin(plugin_path, "Gstring.prc"),
            ...
            with progressbar.ProgressBar(
                prefix='Processing Isomorphic sets (ISO #{variables.processed}/%d, Zeros: {variables.zero_multiplicity_count})' % len(
                    iso_groups),
                ...
                for iso_graphs, (isogs, multiplicity) in zip(FORM_iso_sg_list, graph_it):
                    if iso_graphs[0].name != isogs[0].name:
                        raise FormProcessingError(
                            "Failed to assign the multiplicity factor to the correct graph!")
                    processed_graphs += 1
                    bar.update(processed='%d' % processed_graphs,
                               zero_multiplicity_count='%d' % zero_multiplicity_count)
                    if multiplicity == 0:
                ...
                bar.update(zero_multiplicity_count='%d' %
                           zero_multiplicity_count)
            ...
            if zero_multiplicity_count > 0:
                logger.info(
                    "%d isomorphic sets have 0 multiplicity -> they are dropped!" % zero_multiplicity_count)
        ...
        for i_graph, super_graphs in enumerate(FORM_iso_sg_list):
            super_graphs[0].draw(model, pjoin(
                workspace, '../../Drawings'), FORM_id=i_graph)
            ...
            if isinstance(super_graphs[0].additional_lmbs, list):
                for i_lmb, _, _, sg in super_graphs[0].additional_lmbs:
                    sg.draw(model, pjoin(workspace, '../../Drawings'),
                            FORM_id=i_graph, lmb_id=i_lmb)
    ...
    def write_pySecDec_run(self, output_dir, numerator_input_path, topology_generator_graph, graph_name, graph_id, couplings_prefactor_str, additional_overall_factor, params, model, particle_PDGs):
        ...
        couplings_prefactor_str = re.sub(
            r'rat\((\s*-?\s*\d+),(\s*-?\s*\d+)\)', r'(\1/float(\2))', couplings_prefactor_str)
        couplings_prefactor_str = couplings_prefactor_str.replace(
            '^', '**').replace('i_', '(1j)')
        ...
        processed_params['gs'] = math.sqrt(
            model['parameter_dict']['aS'].real*(4.*math.pi))

        output_path = pjoin(output_dir, 'run_graph_{}.py'.format(graph_id))
        output_numerator_path = pjoin(
            output_dir, 'numerator_{}.txt'.format(graph_id))
        ...
        with open(output_numerator_path, 'w') as num_out:
            ...
            for line in [''.join(numerator_lines),]:  # num_in.readlines():
                ...
                if 'rat' in line:
                    raise FormProcessingError(
                        "Bug when doing rat substitution in the following FORM expression: Original line:\n%s\nProcessed line:\n%s" % (orig_line, line))
                numerator.append(line)
            num_out.write('\n'.join(numerator))
        lorentz_indices = ['mu%d' % (i+1)
                           for i in range(lorentx_indices_count['count'])]
        ...
        default_externals = []
        sig_map = g.get_signature_map()
        all_internal_nodes = set(sum(
            [[e[1], e[2]] for i_e, e in enumerate(g.edge_map_lin) if i_e not in g.ext], []))
        for i_e, e in enumerate(g.edge_map_lin):
            if i_e in g.ext:
                external_edges.append(
                    [e[0], (e[1] if e[1] in all_internal_nodes else e[2])])
                if not all(s == 0 for s in list(sig_map[e[0]][1])[:len(sig_map[e[0]][1])//2]):
                    mass_param = model.get_particle(
                        particle_PDGs[e[0]]).get('mass')
                    if mass_param.upper() != 'ZERO':
                        default_externals.append(
                            (model['parameter_dict'][mass_param].real, 0., 0., 0.))
                    else:
                        ...
                        if len(g.ext) == 2:
                            ...
                            default_externals.append(
                                (1., 0., 0., ((-1)**len(default_externals))*1.))
            else:
                ...
                for i_l, lsig in enumerate(list(sig_map[e[0]][0])):
                    ...
                    if loop_momentum is None:
                        loop_momentum = 'k%d' % (
                            i_l+1) if lsig > 0 else '-k%d' % (i_l+1)
                    else:
                        loop_momentum += '+k%d' % (i_l +
                                                   1) if lsig > 0 else '-k%d' % (i_l+1)
                ...
                for externals_list in [
                    ...
                    for i_p, psig in enumerate(externals_list):
                        ...
                        if loop_momentum is None:
                            loop_momentum = 'p%d' % (
                                i_p+1) if psig > 0 else '-p%d' % (i_p+1)
                        else:
                            loop_momentum += '+p%d' % (i_p +
                                                       1) if psig > 0 else '-p%d' % (i_p+1)
                ...
                if mass_param.upper() == 'ZERO':
                    ...
                    if mass_param not in real_parameters:
                        ...
                        real_parameters_input.append(
                            model['parameter_dict'][mass_param].real)
        ...
        if loop_momenta_str is None:
            ...
            loop_momenta_str = ['k%d' % (i_l+1)
                                for i_l in range(n_loop_momenta)]
            external_momenta_str = ['p%d' %
                                    (i_l+1) for i_l in range(n_externals)]
            for i in range(n_externals):
                for j in range(i, n_externals):
                    replacement_rules.append(
                        ('p%d*p%d' % (i+1, j+1), 'p%d%d' % (i+1, j+1)))
        ...
        repl_dict['additional_overall_factor'] = '1.' + \
            str(additional_overall_factor)
    ...
    def generate_integrand_functions(self, root_output_path, model, additional_overall_factor='',
        ...
        if len(self) == 0:
            raise FormProcessingError(
                "Cannot generat numerators for an empty list of supergraphs.")

        if output_format not in self.extension_names:
            raise FormProcessingError(
                "This FORMSuperGraphList instance requires at least one entry for generating numerators.")
...
"""
        ...
        integrand_type_list = [
            integrand_type] if integrand_type != 'both' else ['PF', 'LTD']
        ...
        for itype in integrand_type_list:
            with progressbar.ProgressBar(
                prefix='Processing %s integrand (graph #{variables.i_graph}/%d, LMB #{variables.i_lmb}/{variables.max_lmb}) with FORM ({variables.timing} ms / supergraph) : ' % (
                    itype, len(self)),
                max_value=len(self),
                ...
                for i_graph, graph in enumerate(self):
                    ...
                    if isinstance(graph[0].additional_lmbs, list) and graph[0].additional_lmbs != []:
                        ...
                        graphs_to_process.extend(
                            [(g.additional_lmbs, i_graph, g) for _, _, _, g in graph[0].additional_lmbs])
                    else:
                        # By setting the active graph to None we will then sum overall members of the iso set.
                        graphs_to_process.append((0, i_graph, None))
                    bar.update(max_lmb='%d' % len(graphs_to_process))
                    for i_lmb, i_g, active_graph in graphs_to_process:
                        bar.update(i_graph='%d' %
                                   (i_graph+1), i_lmb='%d' % (i_lmb+1))
                        i = i_lmb * \
                            FORM_processing_options['FORM_call_sig_id_offset_for_additional_lmb']+i_g
                        ...
                        if output_format in ['raw', 'pySecDec']:
                            ...
                            particle_ids = {e['name']: e['PDG']
                                            for e in characteristic_graph.edges.values()}
                            with open(pjoin(root_output_path, 'raw_SG_expressions', 'numerator_{}.txt'.format(i)), 'w') as f:
                                f.write(self.process_raw_numerator(
                            ...
                            characteristic_graph_dict = self.minimal_dict_representation_of_a_topology_generator_graph(
                                characteristic_graph.squared_topology.topo, particle_ids)
                            characteristic_graph_dict['numerator'] = 'numerator_{}.txt'.format(
                                i)
                            ...
                            with open(pjoin(root_output_path, 'raw_SG_expressions', 'graph_{}.yaml'.format(i)), 'w') as f:
                                f.write(
                                    yaml.dump(characteristic_graph_dict, Dumper=Dumper))

                            if output_format == 'pySecDec':

                                self.write_pySecDec_run(
                                    pjoin(root_output_path, 'pySecDec_runs'),
                                    pjoin(root_output_path, 'workspace',
                                          'out_raw_numerator_{}.txt'.format(i)),
                        ...
                        num = num.replace('\\\n', '')
                        # work around form output bug where the . is disconnected from the floating number
                        num = re.sub(r'(\d)\n *\.', r'\1.', num)
                        # connect functions over a newline
                        num = re.sub(r'\(([^)]*)\n *', r'(\1', num)
                        ...
                        n_incoming = sum(
                            [1 for edge in graph[0].edges.values() if edge['type'] == 'in'])
                        ...
                        for i1 in range(n_tot):
                            ...
                            for i2 in range(i1+1, n_tot):
                                ...
                                for i3 in range(i2+1, n_tot):
                                    m3 = 'p' + \
                                        str(i3+1) if i3 < n_incoming else 'c' + \
                                        str(i3+1-n_incoming)
                                    varmap[str(out_idx)] = 'e_{}_{}_{}_0'.format(
                                        m1, m2, m3)
                                    out_idx += 1
                                    for i4 in range(i3+1, n_tot):
                                        m4 = 'p' + \
                                            str(i4+1) if i4 < n_incoming else 'c' + \
                                            str(i4+1-n_incoming)
                                        varmap[str(out_idx)] = 'e_{}_{}_{}_{}'.format(
                                            m1, m2, m3, m4)
                                        out_idx += 1

                        numerator_header = numerator_header_general + \
                            '\n'.join('#define {} lm[{}]'.format(
                                v, k) for k, v in varmap.items()) + "\n"

                        max_lm = max(int(m.group(1))
                                     for m in lm_pattern.finditer(num))
                        num = re.sub(
                            r'lm(\d*)', lambda m: varmap[m.group(1)], num)

                        conf_secs = num.split('#CONF')
                        for conf_sec in conf_secs[1:]:
                            conf_sec = conf_sec.replace("#CONF\n", '')
                            conf_part = list(conf_exp.finditer(conf_sec))[
                                0].groups()
                            ...
                            denominator_mode = 'NUM' if int(conf[0]) >= 0 else (
                                'FOREST' if len(conf) == 2 else 'DIAG')
                            ...
                            if conf_part[1] == "*tder(2)":
                                ...
                                raise AssertionError(
                                    "Unknown dual configuration: %s" % conf_part[1])
                            ...
                            if denominator_mode == 'DIAG':
                                ...
                                const_secs = const_secs.replace(
                                    '\n', '').replace(' ', '').replace('\t', '')
                                const_code = '*'.join('({})'.format(e)
                                                      for e in const_secs.split(',') if e != '')
                                ...
                                e = [e for e in energy_secs.split(
                                    ',') if e != '']
                                assert (len(e) % 2 == 0)
                                for j in range(0, len(e), 2):
                                    energy_index = int(
                                        e[j][1:])  # skip E prefix
                                    energy_instr = 'sqrt({})'.format(lm_pattern.sub(r'lm[\1]', square_pattern.sub(
                                        r'\1*\1', e[j+1]))).replace('2*', '2.*').replace('4*', '4.*').replace('8*', '8.*')
                                    assert ((cut_id, energy_index) not in energies_per_cut or energies_per_cut[(
                                        cut_id, energy_index)] == energy_instr)
                                    energies_per_cut[(
                                        cut_id, energy_index)] = energy_instr

                                const_code = pow_pattern.sub(
                                    r'pow(\1,\2)', const_code).replace('(1)', '(1.)')
                                const_code = square_pattern.sub(
                                    r'\1*\1', const_code)

                                # convert ints to floats to prevent mpfr and dual number conversion issues
                                const_code = re.sub(
                                    r'([\+\-(])(\d+)\*', r'\1\2.*', const_code)
                                if const_code == '':
                                    const_code = '1.'
                                ...
                                denom_secs = re.sub(
                                    r'(E\d+)\^(\d+)', lambda x: '*'.join([x.group(1)]*int(x.group(2))), denom_secs)
                                denom_secs = re.sub(
                                    r'(lm\d+)\^(\d+)', lambda x: '*'.join([x.group(1)]*int(x.group(2))), denom_secs)
                                denom_secs = re.sub(
                                    r'((?:p|c)\d+_\d+)\^(\d+)', lambda x: '*'.join([x.group(1)]*int(x.group(2))), denom_secs)
                                d = [d for d in denom_secs.split(
                                    ',') if d != '']

                                for j in range(0, len(d), 2):
                                    # strip invd prefix
                                    denom_index = int(d[j][4:])
                                    denom_instr = '1./({})'.format(energy_pattern.sub(r'E[\1]', lm_pattern.sub(
                                        r'lm[\1]', int_pattern.sub(r'\1.', square_pattern.sub(r'\1*\1', d[j+1])))))
                                    assert ((cut_id, denom_index) not in propagators_per_cut or propagators_per_cut[(
                                        cut_id, denom_index)] == denom_instr)
                                    propagators_per_cut[(
                                        cut_id, denom_index)] = denom_instr

                                conf_sec = conf_sec.split('#ELLIPSOIDS')[-1]

                            conf_sec = dot_pattern.sub(r'(\1)*(\2)-(\3)', conf_sec).replace(
                                '*(1)', '*(1.)').replace('=(1)', '=(1.)').replace(')-(0)', ')')

                            const_code = denom_pattern.sub(r'invd[\1]', energy_pattern.sub(
                                r'E[\1]', lm_pattern.sub(r'lm[\1]', const_code)))
                            ...
                            returnval = list(return_exp.finditer(conf_sec))[
                                0].groups()[0]
                            returnval = re.sub(
                                r'(^|[+\-*=]|\*\()(\s*\d+)($|[^.\d\w])', r'\1\2.\3', returnval)

                            base_type = 'complex<double>'
                            dual_base_type = '{}<{}>'.format(
                                dual_num_type, base_type) if dual_num_mode else base_type
                            base_type_f128 = 'complex128'
                            dual_base_type_f128 = '{}<{}>'.format(
                                dual_num_type, base_type_f128) if dual_num_mode else base_type_f128
                            base_type_mpfr = 'mppp::complex'
                            dual_base_type_mpfr = '{}<{}>'.format(
                                dual_num_type, base_type_mpfr) if dual_num_mode else base_type_mpfr

                            if dual_num_mode and all(x not in returnval for x in ['E', 'lm', 'Z']):
                                returnval = '{}({})'.format(
                                    dual_num_type, returnval)

                            if denominator_mode == 'FOREST':
                                conf_sec = return_exp.sub(
                            ...
                            temp_vars = list(
                                sorted(set(var_pattern.findall(conf_sec))))

                            # write out all integer powers as multiplications to prevent slow pow evaluation with floating exponent
                            conf_sec = re.sub(
                                r'pow\(([^,]+),(\d+)\)', lambda x: '*'.join([x.group(1)]*int(x.group(2))), conf_sec)
                            conf_sec = re.sub(
                            ...
                            if denominator_mode == 'FOREST':
                                ...
                                cut_id = int(conf[0])
                                confs.append(
                                    (cut_id, dual_num_type, dual_length))
                                ...
                                if len(temp_vars) == 0:
                                    ...
                                    forests = list(
                                        sorted(set(forest_pattern.findall(conf_sec))))

                                # generate the code for the energies and invds
                                max_energy = max(
                                    (eid + 1 for (cid, eid) in energies_per_cut if cid == cut_id), default=0)
                                energies = [('0' if not dual_num_mode else '{}()'.format(dual_base_type)) if (
                                    cut_id, jj) not in energies_per_cut else energies_per_cut[(cut_id, jj)] for jj in range(max_energy)]
                                max_denom = max(
                                    (eid + 1 for (cid, eid) in propagators_per_cut if cid == cut_id), default=0)
                                denoms = [('0' if not dual_num_mode else '{}()'.format(dual_base_type)) if (
                                    cut_id, jj) not in propagators_per_cut else propagators_per_cut[(cut_id, jj)] for jj in range(max_denom)]
                                ...
                                conf_sec = '\n\t{} E[] = {{{}}};\n'.format(dual_base_type, ('0' if not dual_num_mode else '{}()'.format(dual_base_type)) if len(energies) == 0 else ','.join(energies)) + \
                                           ...
                                           ('\n' + '\n'.join('\t{0} forest{1}=forest_{1}(lm, params, E, invd);'.format(
                                               dual_base_type, f) for f in forests) if len(forests) > 0 else '') + conf_sec

                                main_code = conf_sec.replace('logmUVmu', 'log(mUV*mUV/(mu*mu))').replace(
                                    'logmUV', 'log(mUV*mUV)').replace('logmu', 'log(mu*mu)').replace('logmt', 'log(masst*masst)')
                                if len(temp_vars) == 0:
                                    main_code = forest_pattern.sub(
                                        r'forest\1', main_code)
                                else:
                                    main_code = forest_pattern.sub(
                                ...
                                main_code_mpfr = main_code.replace('pi', 'mppp::real_pi(prec)').replace(
                                    'complex<double>', 'mppp::complex')
                                main_code_mpfr = float_pattern.sub(
                                    r'mppp::real(real128(\1q), prec)', main_code_mpfr)  # TODO: use string instead?
                                main_code_mpfr = main_code_mpfr.replace(
                        ...
                        for p, v in params.items():
                            if 'params' not in str(v):
                                ...
                                integrand_mpfr_main_code = integrand_mpfr_main_code.replace(
                                    p, 'mppp::real({}, prec)'.format(p))

                        for x in range(4):
                            integrand_main_code = empty_line_pattern.sub(
                                r'\n\n', integrand_main_code)
                            integrand_f128_main_code = empty_line_pattern.sub(
                                r'\n\n', integrand_f128_main_code)
                            integrand_mpfr_main_code = empty_line_pattern.sub(
                                r'\n\n', integrand_mpfr_main_code)

                        integrand_main_code = numerator_header + integrand_main_code
                        integrand_f128_main_code = numerator_header.replace(
                            'const std::complex<double> I{ 0.0, 1.0 };', 'constexpr complex128 I{ 0, 1 };') + integrand_f128_main_code
                        integrand_mpfr_main_code = numerator_header.replace(
                            'const std::complex<double> I{ 0.0, 1.0 };', 'const mppp::complex I{ 0, 1 };') + integrand_mpfr_main_code

                        if itype != "PF":
                            integrand_main_code = integrand_main_code.replace('diag_', 'diag_{}_'.format(
                                itype)).replace('forest_', 'forest_{}_'.format(itype))
                            integrand_f128_main_code = integrand_f128_main_code.replace(
...
"""''.format(conf, itype, i, (is_dual + '<') if is_dual else '', '>' if is_dual else '', out_idx,
            ...
            form_factors_input, form_factor_index) for conf, is_dual, dual_length in sorted(x for x in confs)] +
                        ...
                        bar.update(timing='%d' %
                                   int((total_time/float(i_graph+1))*1000.0))
                        bar.update(i_graph+1)

                        writers.CPPWriter(pjoin(root_output_path, '%(header)sintegrand_{}_{}_f64.c' % header_map).format(
                            itype, i)).write((integrand_main_code) % header_map)
                        writers.CPPWriter(pjoin(root_output_path, '%(header)sintegrand_{}_{}_f128.c' % header_map).format(
                            itype, i)).write((integrand_f128_main_code) % header_map)

                        if FORM_processing_options["generate_arb_prec_output"]:
                            writers.CPPWriter(pjoin(root_output_path, '%(header)sintegrand_{}_{}_mpfr.cpp' % header_map).format(
                                itype, i)).write((integrand_mpfr_main_code) % header_map)
        ...
        self.code_generation_statistics['integrand_C_source_size_in_kB'] = int(
            integrand_C_source_size/1000.0)
    ...
    def generate_numerator_functions(self, root_output_path, model=None, additional_overall_factor='', params={},
        ...
        if output_format not in self.extension_names:
            raise FormProcessingError("Output format specified not supported. %s not in %s." % (
                output_format, str(self.extension_names)))

        # copy all the source files before processing the numerators
        if workspace is not None:
            shutil.copy(pjoin(plugin_path, "numerator.frm"),
                        pjoin(workspace, 'numerator.frm'))
            shutil.copy(pjoin(plugin_path, "raw_numerator.frm"),
                        pjoin(workspace, 'raw_numerator.frm'))
            shutil.copy(pjoin(plugin_path, "tensorreduce.frm"),
                        pjoin(workspace, 'tensorreduce.frm'))
            shutil.copy(pjoin(plugin_path, "Gstring.prc"),
        ...
        total_time = 0.
        with progressbar.ProgressBar(
            prefix='Processing numerators with FORM (graph {variables.i_graph}/%d, {variables.timing} ms / supergraph) : ' % len(
                self),
            max_value=len(self),
            variables={'timing': 0, 'i_graph': 0}
        ) as bar:
            ...
            for (graph_index, num_ids, max_buffer_graph, is_zero, timing, code_generation_statistics) in graph_it:
                ...
                bar.update(processed_graph_counter, i_graph=processed_graph_counter, timing='%d' % int(
                    (total_time/float(processed_graph_counter))*1000.0))
...
""".format('\n'.join('#define  {} {}'.format(k, v) for k, v in params.items()))
        ...
        writers.CPPWriter(pjoin(root_output_path, '%(header)snumerator.h' %
                          header_map)).write(header_code % header_map)
        ...
        self.code_generation_statistics['numerator_C_source_size_in_kB'] = int(
            numerator_C_source_size/1000.0)
        ...
        self.code_generation_statistics['generation_time_in_s'] = float(
            '%.1f' % generation_time)
    ...
    def post_process_source_code(self, root_output_path):
        ...
        for fpath in glob_module.glob(pjoin(root_output_path, 'integrand_PF_*_f64.c')):
            match_re = re.match(
                r"integrand_PF_(?P<SGID>\d+)_f64.c", os.path.basename(fpath))
        ...
        for fpath in glob_module.glob(pjoin(root_output_path, 'integrand_LTD_*_f64.c')):
            match_re = re.match(
                r"integrand_LTD_(?P<SGID>\d+)_f64.c", os.path.basename(fpath))
            if match_re is not None:
                SG_ids.add(int(match_re.group('SGID')))
        ...
        dependency_re = re.compile(
            r"^integrand_(?P<integrand_type>[A-Za-z]*)_(?P<SG_id>\d+)(?P<split_id>_s\d+)?_(?P<precision>.*)\.o$")
            ...
            for code_type in ['PF', 'LTD',]:
                ...
                for fpath in all_matches:
                    ...
                    if split_file_name_re.match(os.path.basename(fpath)):
                        continue
                    dependencies.extend(self.split_source_file(root_output_path, os.path.basename(
...
\t$(GPP) --shared -fPIC $(CFLAGS) -o $@ $^""" % {'SGID': SG_id, 'dependencies': ' '.join(formatted_dependencies)})
                ...
                for d in formatted_dependencies:
                    ...
                    d_specs['split_id'] = 0 if d_specs['split_id'] is None else int(
                        d_specs['split_id'][2:])
                    if (d_specs['integrand_type'], d_specs['precision']) in parsed_dependencies:
                        parsed_dependencies[(d_specs['integrand_type'], d_specs['precision'])].append(
                            (d_specs['split_id'], d))
                    else:
                        parsed_dependencies[(d_specs['integrand_type'], d_specs['precision'])] = [
                            (d_specs['split_id'], d), ]
                for k in parsed_dependencies.keys():
                    parsed_dependencies[k] = [d[1] for d in sorted(
                        parsed_dependencies[k], key=lambda v: v[0])]
                sorted_dependency_keys = sorted(list(parsed_dependencies.keys()), key=lambda k: (
                    type_ordering[k[0]], precision_ordering[k[1]]))
                ...
                for (int_type, prec) in sorted_dependency_keys:
                    primary_source_files.append(
                        parsed_dependencies[(int_type, prec)].pop(-1))
                    libs_used_thus_far = []
                    for i_chunk, chunk in enumerate(chunks(parsed_dependencies[(int_type, prec)], FORM_processing_options['max_n_files_in_library'])):
                        libs_used_thus_far.append('FORM_sg_%d_%s_%s_s%d' % (
                            SG_id, int_type, prec, i_chunk+1))
                        repl_dict['all_sg_targets'].append(
...
\t$(GPP) --shared -fPIC $(CFLAGS) -L$(LIBBPATH) %(lib_links)s-o $@ %(dependencies)s""" % {
        ...
        makefile_targets_template = open(
            pjoin(plugin_path, 'Templates', 'FORM_output_makefile_targets.inc'), 'r').read()
    ...
    def split_function(self, function):
        ...
        function_prototype_re = re.compile(
            r'^(?P<type>\S*)\s(?P<name>[^\(]*)\((?P<args>[^\)]*)\)(?P<suffix>.*)$')
        ...
        if function_prototype_match is None:
            raise FormProcessingError(
                "Parsing error when attempting to split function with prototype:\n%s" % function[0])
        ...
        for line in function:
            ...
            if not in_body:
                ...
                if z_def_re.match(line):
                    ...
                    if zs_match:
                        ...
                        max_z_num = max(int(z_i) for z_i in re.findall(
                            'Z(\d+)_', zs_match['zs']))
                        mother_function_header_lines.append(
                            '\t%s Zs_[%d];' % (Z_type, max_z_num+1))
                        continue

                    var_def_match = var_def_re.match(line)
                    if var_def_match:
                        args_for_splits.append((var_def_match.group(
                            'type').strip(), var_def_match.group('name')))
                    mother_function_header_lines.append(process_line(line))
                    continue

            if Z_type is None:
                raise FormProcessingError(
                    "Body of main function to split reached even though Zs definition has not been found yet.")
            ...
            if z_def_re.match(line):
                ...
                if not (line.strip().startswith('*out =') or line.strip().startswith('return ')):
                    raise FormProcessingError(
                        "Function body contained a line that is not a Z definition:\n'%s'" % line)
                mother_function_trail_lines.append(process_line(line))
        ...
        all_args = [tuple(arg.strip().split(' '))
                    for arg in function_prototype_match.group('args').split(',')]

        # Remove the "out" pointer that is of no use for the split functions
        all_args = [a for a in all_args if a[1] != 'out']

        # Add the necessary arguments
        all_args = [arg for arg in args_for_splits if arg not in all_args] + \
            [(Z_type, 'Zs_[]')] + all_args
        if len(set(a[-1] for a in all_args)) != len(all_args):
            raise FormProcessingError(
                "Split function constructed contains arguments with same names but different types: %s" % str(all_args))
        ...
        new_functions.append(mother_function_header_lines +
                             mother_function_body_lines+mother_function_trail_lines)
    ...
    def split_source_file(self, root_output_path, source_code_name, max_n_lines):
        ...
        raw_source = open(
            pjoin(root_output_path, source_code_name), 'r').readlines()

        start_file_re = re.compile(
            r'^// Source post-processed and split into the following files: (?P<dependencies>.*)$')
        ...
        if start_file_match is not None:
            try:
                ...
                raise FormProcessingError("Could not read dependencies from first line of file '%s'." % pjoin(
                    root_output_path, source_code_name))
            return dependencies
        ...
        # and False:
        if (max_n_lines is None or max_n_lines <= 200 or num_lines <= max_n_lines):
            source_code_name_split = source_code_name.split('_')
            # Clear out any additional file that may have existed from a previous run
            split_file_name_template = '%s%s%s' % (
                '_'.join(source_code_name_split[:-1]), '_s*_', source_code_name_split[-1])
            ...
            open(pjoin(root_output_path, source_code_name), 'w').write(''.join(
                ['// Source post-processed and split into the following files: [%s,]' %
                    source_code_name,]+[l for l in raw_source if l != '//NOSPLIT\n']
            ))
        ...
        for line in open(pjoin(root_output_path, source_code_name), 'r').readlines():
                ...
                if was_las_match_a_closing_bracket:
                    ...
                    if in_no_split:
                        ...
                        if not in_function or len(functions) == 0:
                            raise FormProcessingError("Found function end before start in %s: %s" % (
                                pjoin(root_output_path, source_code_name), line))
                        else:
                            in_function = False
            ...
            if no_split_re.match(line):
                if in_function:
                    raise FormProcessingError("No split command found while in the body of a function %s: %s" % (
                        pjoin(root_output_path, source_code_name), line))
                in_no_split = True
            elif in_no_split:
                no_split_C_code.append(line)
                ...
                in_function = True
                functions.append(
                    [line.replace('static', '').replace('inline', '').strip()+'\n',])
            else:
                if not in_function:
                    if len(functions) == 0:
                        header.append(line)
                        ...
                        if line.strip() == '':
                            ...
                            raise FormProcessingError("Error: found out-of-function code that is not a header in %s: '%s'" % (
                                pjoin(root_output_path, source_code_name), line))
                else:
                    functions[-1].append(line)

        # Possibly split functions into smaller subfunctions
        functions = sum([self.split_function(function)
        ...
        source_code_name_split = source_code_name.split('_')
        while True:
            split_file_name = '%s%s%s' % ('_'.join(
                source_code_name_split[:-1]), ('_s%d_' % i_file if i_file > 0 else '_'), source_code_name_split[-1])
    ...
    def generate_squared_topology_files(self, root_output_path, model, process_definition, n_jets, final_state_particle_ids=(), jet_ids=None, filter_non_contributing_graphs=True, workspace=None,
        ...
        if jet_ids is None:
            jet_ids = tuple(list(range(1, 6))+list(range(-1, -6, -1)) +
                            [21, 82, -82]+list(dummy_scalar_PDGs.keys()))
            ...
            for i, g in enumerate(self):
                ...
                if g.generate_squared_topology_files(root_output_path, model, process_definition, n_jets, numerator_call=non_zero_graph,
                    ...
                    topo_collection['topologies'].append({
                        ...
                        'multiplicity': g[0].multiplicity, 'additional_LMBs': [
                            {
                            ...
                            original_lmb_to_other_lmb_affine_transfo, other_supergraph in g[
                                0].additional_lmbs
                        ]
        ...
        open(pjoin(root_output_path, self.name + '.yaml'),
             'w').write(yaml.dump(topo_collection, Dumper=Dumper))

    def produce_output(self):
        form_processor = FORMProcessor(
            self, computed_model, process_definition)
        ...
        if os.path.isfile(pjoin('Templates', 'makefile_user_opts.inc')):
            shutil.copy(pjoin('Templates', 'makefile_user_opts.inc'),
                        pjoin(TMP_FORM, 'makefile_user_opts.inc'))
        else:
            shutil.copy(pjoin('Templates', 'makefile_user_opts_default.inc'), pjoin(
                TMP_FORM, 'makefile_user_opts.inc'))
...
class FORMProcessor(object):
    ...
    def generate_numerator_functions(self, root_output_path, output_format='c', workspace=None, header="", integrand_type=None, force_overall_factor=None, recycle=False, additional_params=None):
        ...
        if force_overall_factor is None:
            ...
            for leg in self.repr_process.get('legs'):
                ...
                helicity_averaging_factor *= len(
                    self.model.get_particle(leg.get('id')).get_helicity_states())
    ...
    def report_generation_statistics(self, root_output_path=None):
        ...
        if root_output_path is not None:
            open(pjoin(root_output_path, 'generation_statistics.txt'), 'w').write(
                pformat(self.super_graphs_list.code_generation_statistics))
    ...
    def compile(cls, root_output_path, arg=[]):
        ...
        if os.path.isfile(pjoin(root_output_path, 'Makefile')):
            try:
                logger.info("Now compiling FORM-generated numerators with options: %s ..." %
                            (' '.join(compile_options['compilation-options'])))

                t = time.time()
                misc.compile(arg=compile_options['compilation-options'],
                             cwd=root_output_path, mode='cpp', nb_core=compile_options["cores"])
    ...
    def generate_squared_topology_files(self, root_output_path, n_jets, final_state_particle_ids=(), jet_ids=None, filter_non_contributing_graphs=True, workspace=None,
        ...
        self.super_graphs_list.generate_squared_topology_files(
            ...
            integrand_type=integrand_type, include_integration_channel_info=(
                FORM_processing_options['include_integration_channel_info'] if include_integration_channel_info is None else include_integration_channel_info),
            selected_cuts=selected_cuts
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--restrict_card',
                        default=pjoin(
                            os.environ['MG5DIR'], 'models', 'sm', 'restrict_no_widths.dat'),
