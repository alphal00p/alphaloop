...
CF f, vx, vec, vec1;
CF subs, configurations, conf, cmb, der, energy, spatial(s), spatialComp(s);
...
L F = gamma(ubarSpinor(1,p1,0),p1,pol(1),pol(2),p2,uSpinor(1,p1,0))*gamma(ubarSpinor(2,p1,0),pol(3),mu1,mu1,pol(4),uSpinor(2,p1,0));
#$epsCount = 0;
#$cepsCount = 0;
#$vCount = 0;
#$vbarCount =0;
...
.sort
#if HELSUM
    repeat id cpol(x?int_,i1?)*pol(x?int_,i2?) = -d_(i1,i2);
    repeat id uSpinor(x?int_,p1?,m?,s1?)*ubarSpinor(x?int_,p1?,m?,s2?) = gamma(s1,p,s2)+m*d_(s1,s2);
    repeat id vSpinor(x?int_,p1?,m?,s1?)*vbarSpinor(x?int_,p1?,m?,s2?) = gamma(s1,p,s2)-m*d_(s1,s2);
    repeat id cpol(x?int_,p1?,0,j1?,i1?)*pol(x?int_,p1?,0,j2?,i2?) = -d_(j1,j2)*d_(i1,i2);
#endif
* count polarizations and spinors 
#do i = 1,50
        if ( occurs(pol) )  $epsCount=$epsCount+1;
        id pol($epsCount,i1?) = vec(eps`i',i1);
        if ( occurs(pol)==0 ) redefine i "50";
        .sort        
#enddo
#do i = 1,50
        if ( occurs(cpol) )  $cepsCount=$cepsCount+1;
        id pol($cepsCount,i1?) = vec(ceps`i',i1);
        if ( occurs(cpol)==0 ) redefine i "50";
        .sort        
#enddo
#do i = 1,50
        if ( occurs(uSpinor) )  $uCount=$uCount+1;
        id uSpinor($uCount,?a,i1?) = spinor(sU`i',i1);
        if ( occurs(uSpinor)==0 ) redefine i "50";
        .sort        
#enddo
#do i = 1,50
        if ( occurs(vSpinor) )  $vCount=$vCount+1;
        id uSpinor($vCount,?a,i1?) = spinor(sV`i',i1);
        if ( occurs(vSpinor)==0 ) redefine i "50";
        .sort        
#enddo
#do i = 1,50
        if ( occurs(ubarSpinor) )  $ubarCount=$ubarCount+1;
        id ubarSpinor($ubarCount,?a,i1?) = spinor(sUbar`i',i1);
        if ( occurs(ubarSpinor)==0 ) redefine i "50";
        .sort        
#enddo
#do i = 1,50
        if ( occurs(vbarSpinor) )  $vbarCount=$vbarCount+1;
        id ubarSpinor($vbarCount,?a,i1?) = spinor(sVbar`i',i1);
        if ( occurs(vbarSpinor)==0 ) redefine i "50";
        .sort        
#enddo
.sort
*Print;
...
Multiply counter(1+$cepsCount+$epsCount);
...
#do i=1,1
    id  gamma(x?int_,?a, p?vector_,  y?int_) = sum_(iter,1,4,gamma(x,?a,iter)*slash(iter,y,p));
    id  gamma(x?int_,?a, intSym?int_, y?int_) = sum_(iter,1,4,gamma(x,?a,iter)*gam(iter,y,intSym));
    id  gamma(x?int_, p?vector_,  y?int_) = slash(x,y,p);
    id  gamma(x?int_, intSym?int_,  y?int_) = gamma(x,y,intSym);
