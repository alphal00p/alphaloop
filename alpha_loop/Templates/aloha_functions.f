...
C distribution.
C
C File customized for the alphaLoop output.
C
C###############################################################################


C###############################################################################
C Subroutine specialised for emulating the cut numerator in LTD^2
     ...
     &     pp,pp3,sqp0p3,sqm(0:1)
      ...
      double precision rZero, rHalf, rTwo, pE, px, py, pz
...
C     TODO
      fi(3) = DCMPLX(0.0d0,0.0d0) 
      fi(4) = DCMPLX(0.0d0,0.0d0)
      fi(5) = DCMPLX(0.0d0,0.0d0)
      fi(6) = DCMPLX(0.0d0,0.0d0)
...
C         \slashed{p}.{1,0,0,0} = {pE,0,pz,px+i py}
          if (nhel.eq.0) then
             fi(3) = DCMPLX(pE,0.0d0)
             fi(4) = DCMPLX(0.0d0,0.0d0)
             fi(5) = DCMPLX(pz,0.0d0)
             fi(6) = DCMPLX(px,py)
C         \slashed{p}.{0,1,0,0} = {0,pE,px-i py,-pz}
          elseif (nhel.eq.1) then
             fi(3) = DCMPLX(0.0d0,0.0d0)
             fi(4) = DCMPLX(pE,0.0d0)
             fi(5) = DCMPLX(px,-py)
             fi(6) = DCMPLX(-pz,0.0d0)
C         \slashed{p}.{0,0,1,0} = {-pz, -px -i py, -pE, 0}
          elseif (nhel.eq.2) then
             fi(3) = DCMPLX(-pz,0.0d0)
             fi(4) = DCMPLX(-px,-py)
             fi(5) = DCMPLX(-pE,0.0d0)
             fi(6) = DCMPLX(0.0d0,0.0d0)
C         \slashed{p}.{0,0,0,1} = {-px + i py, pz, 0, -pE}
          elseif (nhel.eq.3) then
             fi(3) = DCMPLX(-px,py)
             fi(4) = DCMPLX(pz,0.0d0)
             fi(5) = DCMPLX(0.0d0,0.0d0)
             fi(6) = DCMPLX(-pE,0.0d0)
          endif
C         Add the -m*1 piece of the numerate (here this is an
C         antifermion)
          if (fmass.ne.rZero) then
             fi(3+nhel) = fi(3+nhel) - DCMPLX(fmass,0.0d0)              
          endif
      else
          fi(3+nhel) = DCMPLX(1.0d0,0.0d0)
      endif
...
C     TODO
      fo(3) = DCMPLX(0.0d0,0.0d0)
      fo(4) = DCMPLX(0.0d0,0.0d0)
      fo(5) = DCMPLX(0.0d0,0.0d0)
      fo(6) = DCMPLX(0.0d0,0.0d0)
...
C         \slashed{p}.{1,0,0,0} = {pE,0,pz,px+i py}
          if (nhel.eq.0) then
             fo(3) = DCMPLX(pE,0.0d0)
             fo(4) = DCMPLX(0.0d0,0.0d0)
             fo(5) = DCMPLX(pz,0.0d0)
             fo(6) = DCMPLX(px,py)
C         \slashed{p}.{0,1,0,0} = {0,pE,px-i py,-pz}
          elseif (nhel.eq.1) then
             fo(3) = DCMPLX(0.0d0,0.0d0)
             fo(4) = DCMPLX(pE,0.0d0)
             fo(5) = DCMPLX(px,-py)
             fo(6) = DCMPLX(-pz,0.0d0)
C         \slashed{p}.{0,0,1,0} = {-pz, -px -i py, -pE, 0}
          elseif (nhel.eq.2) then
             fo(3) = DCMPLX(-pz,0.0d0)
             fo(4) = DCMPLX(-px,-py)
             fo(5) = DCMPLX(-pE,0.0d0)
             fo(6) = DCMPLX(0.0d0,0.0d0)
C         \slashed{p}.{0,0,0,1} = {-px + i py, pz, 0, -pE}
          elseif (nhel.eq.3) then
             fo(3) = DCMPLX(-px,py)
             fo(4) = DCMPLX(pz,0.0d0)
             fo(5) = DCMPLX(0.0d0,0.0d0)
             fo(6) = DCMPLX(-pE,0.0d0)
          endif
C         Add the +m*1 piece of the numerate (here this is a
C         fermion)
          if (fmass.ne.rZero) then
             fo(3+nhel) = fo(3+nhel) + DCMPLX(fmass,0.0d0)              
          endif
      else
          fo(3+nhel) = DCMPLX(1.0d0,0.0d0)
      endif
...
C     TODO
      vc(3) = DCMPLX(0.0d0,0.0d0)
      vc(4) = DCMPLX(0.0d0,0.0d0)
      vc(5) = DCMPLX(0.0d0,0.0d0)
      vc(6) = DCMPLX(0.0d0,0.0d0)

C     Implement \sum_h \eps_h \eps^\star_h = -g^{\mu\nu}
C     (valid because we consider external ghosts)
      if (leftRight.eq.1) then
          if (nhel.eq.0) then
              vc(3) = DCMPLX(-1.0d0,0.0d0)
          else
              vc(3+nhel) = DCMPLX(1.0d0,0.0d0)
          endif
      else
          vc(3+nhel) = DCMPLX(1.0d0,0.0d0)
      endif
