...
S aa,bb,cc,dd,ii,xx, cfCol,caCol, Tf;
...
* 0. case: Outside of trace
            id ifmatch->end  TCol(cOli1?,?aa,cOlj1?,?bb,cOli2?)*TCol(cOli3?,?cc,cOlj1?,?dd,cOli4?) = 1/2*( TCol(cOli1,?aa,?dd,cOli4)*TCol(cOli3,?cc,?bb,cOli2)- 2*cfCol/(cOlNF^2-1)*TCol(cOli1,?aa,?bb,cOli2)*TCol(cOli3,?cc,?dd,cOli4));
            id ifmatch->end TCol(cOli1?,?aa,cOlj1?,?bb,cOlj1?,?cc,cOli2?) = 1/2*( TCol(cOli1,?aa,?cc,cOli2)*TrCol(?bb)-2*cfCol/(cOlNF^2-1)*TCol(cOli1,?aa,?bb,?cc,cOli2));
* 1. case: within trace
            id ifmatch->end TrCol(?aa,cOlj1?,?bb,cOlj1?,?cc)=1/2*(TrCol(?aa,?cc)*TrCol(?bb)-2*cfCol/(cOlNF^2-1)*TrCol(?aa,?bb,?cc) );
            id ifmatch->end TrCol(?aa,cOlj1?,?bb)*TrCol(?cc,cOlj1?,?dd) =  1/2*( TrCol(?aa,?dd,?cc,?bb)- 2*cfCol/(cOlNF^2-1)*TrCol(?aa,?bb)*TrCol(?cc,?dd));
* 2. case: outside and inside trace
            id ifmatch->end TCol(cOli1?,?aa,cOlj1?,?bb,cOli2?)*TrCol(?cc,cOlj1?,?dd) = 1/2*( TCol(cOli1,?aa,?dd,?cc,?bb,cOli2) - 2*cfCol/(cOlNF^2-1)*TCol(cOli1,?aa,?bb,cOli2)*TrCol(?cc,?dd));         
            goto end2;
            label end;    
            redefine loop "0";
            label end2;
