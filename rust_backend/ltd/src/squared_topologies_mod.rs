...
impl SquaredTopologySet {
    ...
    pub fn evaluate<'a, T: FloatLike>(
        ...
        //let mut k_cut_fix = LorentzVector::default();
        ...
        for i in 0..n_loops {
            ...
            //  |  5.0000000000000000e+02  0.0000000000000000e+00  0.0000000000000000e+00 -5.0000000000000000e+02
            //  |  4.9999999999999977e+02  1.0740197658523471e+02  4.3070555989194781e+02 -1.9321629357729731e+02
            //  |  5.0000000000000011e+02 -1.0740197658523471e+02 -4.3070555989194793e+02  1.9321629357729719e+02
            if i==n_loops-1{
                // k_cut_fix = LorentzVector::from_args(
                //     T::zero(),
                //     <T as NumCast>::from(rot[0][0]).unwrap() * l_space[0]
                //         + <T as NumCast>::from(rot[0][1]).unwrap() * l_space[1]
                //         + <T as NumCast>::from(rot[0][2]).unwrap() * l_space[2],
                ...
                // *(k_cut_fix.spatial_squared() +  <T as NumCast>::from(25.).unwrap() ).powi(3).inv()
                // *Into::<T>::into(500. as f64)
                // *Into::<T>::into( 1./9.8696044010893586188344909998762);


                l_space[0] = <T as NumCast>::from(1.0740197658523471e+02).unwrap() ; 
                l_space[1] = <T as NumCast>::from(4.3070555989194781e+02).unwrap() ; 
                l_space[2] = <T as NumCast>::from(-1.9321629357729731e+02).unwrap() ;                 
